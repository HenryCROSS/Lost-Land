//! 「种族 × 职业」合成图：本体 9 个种族 × 13 个职业 = 117 张。
//!
//! **〔2026-08-31，批次 24〕从 52 张变成 117 张，本文件的绘制代码一个
//! 字都没改**——五个新种族只往 [`npc::race_bodies`] 加了五行，名字与图
//! 都是两张表的笛卡尔积现拼的。本文件唯一跟着变的是
//! `合成图的数量等于种族数乘职业数` 里那个字面量（52 → 117），它是一条
//! 刻意留的绊线：真相源是两张表的长度，那个数字只是「有人偷偷动了表」
//! 时会响的第二道铃。下面「加第 5 个种族要补 13 张，但补的是 13 行
//! `RACE_BODIES` 里已经有的东西」那句话因此不再是预言，是实测。
//!
//! # 所有者裁定与它推翻了什么
//!
...
//!
//! 因此这 117 张不是「抄 117 遍人形」，而是**在既有配方之上再叠两个维度**：
...
//! **不打任何日志**，117 张图会一张都用不上而屏幕上毫无异常（`skin.rs`
...
/// 117 个名字是两张表的笛卡尔积**现拼**的，写死成字面量等于把同一份
...
mod tests {
    ...
    fn 合成图的数量等于种族数乘职业数() {
        ...
        assert_eq!(names.len(), 117, "本体现有 9 个种族 × 13 个职业");
    }
    ...
    fn 职业只换衣服不换身体() {
        ...
        // 一层如果把它们盖掉，同一个职业的九个种族就分不开了，而那正是
