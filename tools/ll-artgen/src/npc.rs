...
//! 9 个种族 × 13 个职业 = 117 种组合。逐个组合画一张图，加第 10 个种族
//! 要补 13 张、加第 14 个职业要补 9 张——是乘法级的负担。
//!
//! **〔2026-08-31，批次 24〕这段乘法从 4×13=52 变成了 9×13=117**，而
//! 本模块与 [`crate::composite`] 的绘制代码**一个字都没改**——加的是
//! [`RACE_BODIES`] 里五行、每行八个数字。这正是下面那句话第一次被真的
//! 检验：配方数仍然只有「种族数 + 职业数」。
//!
//! **所有者后来裁定就要那一整张乘法表**（「我觉得还是每个种族的每个
//! 职业画上风格不同的图片。虽然美术资源会很多。」），落点是
//! [`crate::composite`]。但那道乘法题并没有消失，只是换了个答案：合成图
//! **复用本模块的配方**（[`draw_race_body`] 一行没重写、
//! [`draw_profession_badge`] 直接叠上去），117 张图仍然只有「种族数 +
//! 职业数」份配方。
//!
...
//!
//! 本模块的资产量因此是 `种族数 + 职业数`（9 + 13 = 22 张）；
//! [`crate::composite`] 另外产出 `种族数 × 职业数`（117 张），但它靠的
...
/// 本体九个种族的身子配方，顺序即 `mods/lostland/races.json5` 的声明
/// 顺序。名字是种族 id 的**本地名**，见模块文档。
const RACE_BODIES: [(&str, BodySpec); 9] = [
    (
    ...
    // ────────────────────────────────────────────────────────────────
    // 五个新种族（批次 24）。数值那一侧由所有者批准，见
    // mods/lostland/races.json5；这里只管**长什么样**。
    //
    // # 加一个种族就是加八个数字，这一条在这里第一次被真的检验
    //
    // 本模块文档从落地起就写着「加一个种族是加八个参数，不是抄一遍
    // 那几十行」（ADR 0021「抽象的正当理由是有算法要共用」）。此前
    // 那是一句没被兑现过的承诺——四族是同一批写下来的。本批把种族数
    // 从 4 加到 9、合成图从 52 加到 117，`draw_race_body`、
    // `composite.rs` 的 `draw_composite`、`PROFESSION_TOOLS` 三处
    // **一个字都没有改**。
    //
    // # 八个参数怎么选：几何必须与配色一起变
    //
    // 门槛是 `四个种族的身子两两之间至少四分之一像素不同`（96/384），
    // 而**合成图那一侧的门槛更紧**：合成图把躯干、帽子、袖子上半段
    // 全换成职业色，种族只剩「腿 + 脚 + 脸 + 耳 + 胡子 + 袖口以下的
    // 小臂」这几块，逐个数下来约 94 像素——**恰好在 96 门槛之下**。
    // 也就是说：**只换配色的两个种族，穿上同一套职业衣服之后会当场
    // 跌破门槛**。
    //
    // 因此下面五族每一个的
    // `(head_top, shoulder_w, leg_top, ears, beard_h)` 五元组都与既有
    // 四族、也与彼此**互不相同**——身高、体宽、腿长任意一项一变，
    // 整块躯干与四肢的坐标就跟着平移，差异量立刻远离门槛。这不是
    // 装饰性的多样化，是这条断言能不能过的结构性前提。
    // ────────────────────────────────────────────────────────────────
    (
        // 骆驼人：高而宽（与矮人同宽但高两格），沙色皮、土黄发、
        // 赭石衣，留一撮短须。沙漠那一支在远景里靠「大块土黄」
        // 认出来。
        "camelfolk",
        BodySpec {
            skin: (208, 182, 128),
    ...
    (
        // 欧克：九族里最魁梧的一支——最高的那一档配最宽的肩，
        // 灰绿皮、招风耳、下颌一圈短须。与哥布林同为绿皮但**不同
        // 色调也不同体型**（哥布林是最矮最瘦的黄绿，欧克是最高最宽
        // 的灰绿），远景里不会看错。
        "orc",
        BodySpec {
            skin: (108, 132, 102),
