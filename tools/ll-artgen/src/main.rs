...
/// 本体新增的松散贴图：九个种族的身子 + 十三个职业的挂件 + 气候条带
...
fn loose_only_entries() -> Vec<LooseOnlyEntry> {
    ...
    // 「种族 × 职业」合成图（117 张）。锚点/占地与种族身子那一档完全
...
mod tests {
    ...
    #[test]
    fn 六帧行走循环里最突兀的一跳落在从不连播的两帧之间() {
        // 这条断言的性质是：**六个姿态里差异最大的那一对，永远不会被
        // 连着播出来**。等价于「循环里每一步都比『最远的两个姿态直接
        // 互跳』更平滑」——也就是六帧方案存在的全部理由。同时每一对
        // 相邻帧的差异必须大于 0，确保六帧不是复制粘贴出来的重复贴图。
        //
        // # 判据为什么是现算的
        //
        // 此前这里写的是硬编码的 `diff < 32`（两帧方案那次硬切在**当时
        // 那套美术**下的实测值）。玩家贴图重画之后那个数当场过期：新
        // 的双腿是绕中线镜像摆动的，`hero_walk_0` 与 `hero_walk_1` 两条
        // 腿的**位置集合完全一致**，只是前后腿的深浅对调，硬切反而成了
        // 全场最小的一步（实测 48），而相邻的 `walk_0 → walk_2` 两条腿
        // 各挪两像素、差异 72。
        //
        // 一个要人手动才能跟上美术的数，表达的就不再是任何性质，只是
        // 「上次跑出来是多少」。改成从六帧自己现算的极值之后，判据跟着
        // 美术一起活。
        // Arrange
        ...
        // 六个姿态里差异最大的那一对（十五对全比一遍），现算。
        let worst = (0..frames.len())
            .flat_map(|i| (i + 1..frames.len()).map(move |j| (i, j)))
            .map(|(i, j)| count_differing_pixels(&frames[i], &frames[j]))
            .max()
            .expect("六帧恒有十五对");
        assert!(worst > 0, "六帧不该是同一张图");
        ...
        for i in 0..frames.len() {
            ...
            assert!(
                diff < worst,
                "{} 与 {} 差异 {diff} 像素，正是六个姿态里最远的那一跳（{worst}）——\
                 最突兀的一对不该被连着播出来",
                WALK_CYCLE_ORDER[i],
                WALK_CYCLE_ORDER[next]
            );
