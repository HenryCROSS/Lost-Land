...
**尚未创建**。〔2026-08-23 补注：其中 **`ll-script` 后来创建了、又整体删除了**——Steel
脚本系统已随脚本系统拆除批次移除，规格 §5 的终态依赖链里不再有这一环，见规格 §4 的
`[2026-08-23 规格修订]`。本文档其余部分仍是 2026-08-17 的冻结快照，漂移见
[discrepancies.md](discrepancies.md)。〕这不是遗漏，是项目当前处于 P3（回合与战斗层）阶段，规格 §15 的阶段表把这些 crate
