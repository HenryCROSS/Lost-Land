...
| **5** ✅ | 交易（**2026-09-01 已落地**，计划文档 `docs/superpowers/plans/2026-09-01-batch31-dialogue-trade.md`） | 本行预告的四项**全部照做**：`open-trade` 不产 `Effect`（`resolve` 里那一支是空的，UI 侧由 `ll_sim::dialogue::DialogueOutcome::is_ui_only` 决定「不提交意图」）、`Intent::Trade`（`ll_sim::trade` + `ll_sim::resolve::trade`）、NPC 初始钱包（`ll_mod::npc_wallet`，人口 × 职业档位，**`EXPECTED_POPULATED_WORLD_DIGEST` 重冻**）、占位价格公式（基础价 × 归属系数，`ItemAttrs::base_price` 的第一个结算侧消费者）。**两处收窄**：`Intent::Trade` **不带 `count`**（一次一件，与批次 4 的 `give-item` 同一条理由）；**交易屏压过 `next`**，退出回到世界而不是刚才那段对话（`Modal` 只有一块屏、不是栈）。**一条本批自裁**：规格没说交易消不消耗回合，取「**不消耗**」——加一条 `ScheduleNext` 是一行，撤一条要动回放摘要。owner 校验**复用**批次 4 那一条（`may_give_away` 搬进 `ll_sim::ownership` 并泛化），不另写一份 |
| **6** ✅ | NPC 姓名（**2026-09-02 已落地**，计划文档 `docs/superpowers/plans/2026-09-01-batch34-npc-names.md`） | `CultureAttrs.naming` + 渲染期现算；对话文案从职业名换成 `{ $npc_name }`——**只改 `.ftl`，一个 JSON5 字都不改**（这正是本批 5.4 那条边界白送的好处）。**落地实测：`mods/lostland/dialogues.json5` 逐字未动**，改的只有 `assets/locales/*.ftl` 两条开场白。一处收窄：只做给定名不做姓氏（`FamilyId` 在厚层零来源）；一处超出预告：多语言音素表按下标对齐 + 注册期表长校验，那是 `naming-and-localization.md` 三节要求的 |
