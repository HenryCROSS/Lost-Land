...
> **【2026-09-01 再更正（批次 31，对话系统的批次 5）】上面这段原文预告的
> 那一对，现在也在了。**
> `open-trade` 落地，它与 `JoinSettlement` **都不带参数**，两条字节流长度
> 从此相同 ⇒ `JoinSettlement` 自己的判别值第一次可观察。实测：把
> `OpenTrade` 的 `write_u64(4)` 改成 `write_u64(1)`（与 `JoinSettlement`
> 撞号），`后果种类不同的两个对话节点摘要不同` **当场红**。
> 因此上面那句「`JoinSettlement` 自己的判别值仍然观察不到」**就此作废**；
> 那条「必须往哈希里写点什么」的兜底断言**仍然保留**——它守的是「整支
> 空过」，与撞号是两件事。本节按纪律第 9 条原文一字不改，只加标记；
> 更正方与完整证据见
> [批次 31 计划](2026-09-01-batch31-dialogue-trade.md) 与
> `crates/ll-mod/src/content_hash.rs` 里
> `后果种类不同的两个对话节点摘要不同` 的文档注释。
