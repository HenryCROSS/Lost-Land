...
## 八、更正（2026-08-27，所有者实机试玩后追加）

**本节是追加的更正段，上面正文一个字都没改**（本仓库惯例，见
`knowledge/design/race-system.md` 末尾几节的写法）。读到本文档时，**第五节第 5 条
与第四节 D5 表格的读法已经被下面这条裁定推翻**，正文保留是为了让人看得见改过什么。

### 所有者裁定原话

> 「关于矮人那个，整体机制应该是**只要有一方处于敌对状态，另一方也会发起攻击**」

### 改了什么

`crates/ll-sim/src/ai_query.rs` 的 `culture_declares_hostile` 从**有向**判据改为
**对称**判据：

| | 落地时（有向） | 更正后（对称） |
|---|---|---|
| 发起者有文化 | `hostility(发起者, 目标)` | `max(hostility(A, B), hostility(B, A))` |
| 发起者无文化 | `hostility(目标, 哨兵)` | 同上，无文化那一方用哨兵索引参与**两个**方向 |

「无文化」哨兵的角色没有变：某一方身上找不到 `Culture` 归属时才顶上，两侧都有真实
文化时它完全不参与（`哥布林撞哥布林不敌对` 那条守着这一点）。

### 第五节第 5 条被反转，不是删掉

- **旧**：`矮人矿工撞哥布林不是攻击`（3 < 5）——理由是「守那份刻意的不对称」。
- **新**：`矮人矿工撞哥布林也是攻击`（`max(3, 6) = 6 ≥ 5`）。

D4 的判据草图写的本来就是「第一项**对称**取两个方向的最大值」，而第五节第 5 条写的
是有向预期——**正文这两处当时就互相矛盾，实现选了后者**。本次更正把两处统一到
「对称」这一侧，与 D4 原文一致。

### 那个刻意写低的 3 没有作废，只是换了一层

`mining_hold → goblin_warband = 3` 一个字没改，**编年史层的战争推演也一个字没改**
（`crates/ll-world/src/chronicle.rs` 的 `hostility_between`/`wage_wars`/`pick_target`
仍然按 `攻方 → 守方` 有向读表）。分层是：

| 层 | 方向性 | 回答的问题 |
|---|---|---|
| 内容声明（`CultureAttrs::hostility`） | **有向** | 「A 有多恨 B」 |
| 编年史战争推演 | **有向** | 「A 会不会**出兵**打 B」——矮人仍然不太会 |
| 实体级撞格路由（`declared_hostile`） | **对称** | 「我走进你这一格意味着什么」 |

两者不矛盾：出兵是集体决策，可以单方面隐忍；迎面遭遇不是——一方拔刀，另一方不可能
站着挨打。**同一张表是这两个问题共同的输入，不是其中任何一个的答案。**

### D5 表格的对称读法

| 配对（无序） | 两个方向 | 撞格路由敌对？ |
|---|---|---|
| 无文化 ↔ `goblin_warband` | 0 / 6 | **是** |
| `mining_hold` ↔ `goblin_warband` | 3 / 6 | **是**（本次改变的就是这一格） |
| `farmstead` ↔ `goblin_warband` | 0 / 4 | 否 |
| `stonecutters` ↔ `goblin_warband` | 0 / 4 | 否 |
| 无文化 ↔ `farmstead` | 0 / 0 | 否 |

阈值 `HOSTILE_CULTURE_THRESHOLD = 5` 一个字没改。
