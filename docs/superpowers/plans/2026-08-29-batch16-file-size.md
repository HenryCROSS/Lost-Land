...
**测试**：原计划不动文件内的 `mod tests`（1622 行）。**实际动了**，理由是被迫的：
新建 `src/app/` 目录之后，`rustfmt` 不再按「文件名 + 内联模块名」两层算嵌套
`#[path]` 的基准目录，`mod tests` 里那句 `#[path = "../../app_navigation_tests.rs"]`
当场解析不到（`rustc` 仍然找得到），`cargo fmt` 直接报错。把 `mod tests` 整体外挂成
`app_tests.rs` 之后，`mod navigation_tests` 变成**非内联**模块，基准就是本文件所在
目录，与既有的 `mod save_tests` 完全同一种写法，两个工具都认——一处修改同时解决了
分歧与行数。`app_save_tests.rs`、`app_navigation_tests.rs` 两个文件本身不动。
...
| 项 | 改前 | 改后 |
|---|---|---|
| `run_all.sh` | exit 0 | **exit 0** |
| 测试数 | 2798 通过 / 0 失败 | **2798 通过 / 0 失败** |
| `EXPECTED_WORLD_DIGEST` | `11_270_479_921_196_970_914` | **逐位不变** |
| `EXPECTED_REPLAY_DIGEST` | `11_222_878_776_777_704_235` | **逐位不变** |
| `CONTENT_HASH_ALGORITHM_VERSION` | `27` | **不变** |
| 超限文件数 | 17 | 17（换了两张面孔：`resolve.rs`、`app.rs` 出榜，两个断言文件进榜） |

`resolve.rs` 4246 → **487** 代码行，九个子模块 112–392；
`app.rs` 2842 → **572** 代码行（其中产品代码由约 1787 降到 572），五个子模块 137–617。

### 计划之外、实际必须做的四件事

1. **124 条 rustdoc 内链的目标路径**跟着项走（显示文本一个字不变）。不改 `check_doc_links.sh` 会红。
2. **`check_field_consumers.py` 的决策层 glob** 从 `src/*.rs` 改成 `src/**/*.rs`。
   非递归通配当场漏掉整个 `resolve/` 目录，26 个本来接了线的字段一起变成假阳性。
   扫描到的文本集合与拆分前完全相同。
3. **`app.rs` 的 `mod tests` 被迫外挂**，见 §B2。
4. **两处测试文件加了 `use`**（`resolve_tests.rs` 的导入块、`app_navigation_tests.rs` 的
   `screen_data`）——`use super::*` 只带得走父模块**自己仍在使用**的名字。**断言一个字未改**，
   已用「折叠全部空白后逐字符比对」逐条复核。
