...
9. ~~**玩家初始钱包仍然是 0**（`build_player_agent` 没动）。所有者裁定第 4 条说的是 NPC。
   ...
   > **【2026-09-02 已裁定并落地，本条作废（原文划掉保留，追溯用）】**
   > 所有者裁定 **1000**。落点 `ll_game::world::PLAYER_STARTING_WALLET`
   > ← `build_player_agent`。「玩家开局只能卖不能买」这条后果就此消失：
   > 开局那笔钱刚好买得起一件中等价位的货（本体烤肉 `base_price` 900）、
   > 买不起两件，量级与 NPC 那一侧（`ll_mod::npc_wallet`）一致。
   > `EXPECTED_POPULATED_WORLD_DIGEST` 随之重冻（四步证据写在那条常量的
   > 文档里），`CURRENT_SCHEMA_VERSION` **不动**，**老存档不受影响、不写
   > 迁移**——与本节第 7 条对 NPC 钱包做的判断逐字相同，且这一次是实测的
   > （`crates/ll-game/tests/player_wallet.rs` 的
   > `老存档里的钱包按存档里的值读回来`）。
