...
1. `intent.rs:9` 是单纯的编号写错，**应改为 C3**（字面完全对应，无需解释）。
2. HashMap/HashSet 迭代顺序规则**字面上不属于 C1–C4 中任何一个**，但若必须从现有四个里选一个最接近的，**C3 更站得住脚**：C3 的落脚点句「否则并行化与读档重放都会分叉」和 HashMap 迭代顺序失控的后果（同一份存档/同一个 Intent 流在不同运行间产出不同结果）是同一个失败模式，只是触发源不同（RNG 流 vs 容器迭代）；C4 的落脚点是「后台推进要到确定 tick」，与容器迭代顺序完全不相关，选 C4 缺乏文本依据。**建议五处 HashMap 相关引用统一改标为 C3**，同时在规格 §4 C3 的表述里补一句「本约束同样禁止 HashMap/HashSet 迭代顺序参与任何逻辑判断——道理相同：任何非确定性的隐藏输入源都会导致并行化与读档重放分叉，RNG 流只是最容易犯错的一种」，把这条已经在五处代码里事实生效的规则正式收编进规格文本，而不是继续放任它是一条没有条文依据的口头约定。规格修订不在本工单范围内（依权限边界，规格文件本次任务另有专门的第一步在改，改动应合并考虑，避免同一份文件被两处工作分别改出冲突）。

**应改为**：

```
crates/ll-world/src/state.rs:196        C3 → 不变（已经是建议编号，仅需等规格补充表述后去掉"约束"二字后的引号内容与规格原文保持一致）
crates/ll-sim/examples/p3_acceptance/turn.rs:356   C3 → 不变，同上
crates/ll-mod/src/registry.rs:66        C4 → 改为 C3
crates/ll-mod/src/topo.rs:5             C4 → 改为 C3（含模块文档标题「确定性」一节正文）
crates/ll-world/src/entity/id.rs:30     C4 → 改为 C3
crates/ll-sim/src/intent.rs:9           C4 → 改为 C3（此处是独立的字面对应错误，不属于「选哪个更合理」的判断，是纯粹改正）
```

**风险**：六处均为注释/文档字符串改动，不改变任何运行时行为，编译不受影响，零测试影响。真正的风险在于**不改**：只要这条规则继续没有正式条文，新代码继续会有人凭感觉标 C3 或 C4，问题只会越积越多；建议这份工单被执行时，一并检查是否还有本次搜索未覆盖到的新增引用（本次搜索范围是 `crates/**/*.rs` 全文 `grep -rn "约束 C\[0-9\]\|规格 C\[0-9\]"`，如实记录搜索命令供复核）。
