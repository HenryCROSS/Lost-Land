...
## 【2026-08-31 订正】本 ADR 两处「ADR 0018」编号有误，应为 ADR 0022

**正文一字未改**（ADR 惯例，见
[0018](0018-engine-layer-vs-gameplay-layer-scripting-boundary.md)「与 0016 的关系」
一节论证的同一条理由）。

- **哪两处**：本文「后果」一节第 2 条「……只能写成『拿不到就跳过』的假测试，
...
- **为什么错**：ADR 0018 讲的是**引擎层／玩法层的脚本边界**（三步归类判据 +
  ...
  它正文里就有「故意改坏」验证的原始记录（`sea_level` 400 → 500 → 400，确认
  哈希确实变化），并把自己的主张概括成「覆盖不全的守护等于没有守护」。
- **结论现在是什么**：本 ADR 那两处的**论证与结论完全成立、一字不改**——
  「拿不到适配器就跳过」的测试是一条永远绿的假守护，正是 0022 点名的失效模式；
  错的只是编号，依据是 **ADR 0022**。
