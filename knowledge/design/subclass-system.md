...
**2026-08-22 订正**（由 `crafting-system.md` 触发，基线提交 `e81e03c`）：一节候选表「工匠/锻造」
与「厨艺」两行的判断、四节 `SubclassUnlockTrigger` 的变体集合、六节排序的第 3/5/8 三项，
均已按制作系统的核实结论修订——修订处就地标注 **【2026-08-22 订正】**，原文保留以便对照。
...
| 厨艺 | `RecipeDef` + `Satiety`（设计） | 原描述准确。**补充一处会削弱这条方向价值的细节**：`food-and-cooking-system.md` 已经裁定「菜谱全部已知不设解锁门槛」——副职在这条方向上不能靠「解锁菜谱」当奖励，只能靠「解锁副职专属技能」（如野外烹饪），价值比炼金弱一档 <br>**【2026-08-22 订正】** 这条「弱一档」的判断已被 `crafting-system.md` 七节的**类别闸门**部分推翻：闸门挂在配方**类别**而不是单条配方上，内容设计因此可以让基础烹饪类别人人可做（食物系统五节裁定原样成立）、另设一个要求厨艺副职的高阶类别。厨艺不再天然弱一档 |
...
**一处订正解决不了的隐忧**：成长挂钩回答的是「怎么获得这个副职」，不回答「获得之后它还有没有
用」。项目所有者已裁定**只有装备武器才有耐久**——武器会磨损，玩家会反复回来找工匠；衣服永远不
坏，裁缝的每件产出都是一次性买卖。这条长期需求的不对称需要「耐久适用范围」或「物品能否授予
抗性」的裁定，两者都不在本文档也不在 `crafting-system.md` 的范围内。
...
pub enum SubclassUnlockTrigger {
    ...
    TilesExplored,
    /// 【2026-08-22 订正新增，见一节「订正」】累计制作某个配方类别的
    /// 成品达到 threshold 次——挂载点是 crafting-system.md 八节的
    /// craft_progress_effects，结构同样照抄 kill_progress_effects。
    /// ContentIndex 指向 RecipeCategoryTable（不是某一条具体配方），
    /// 让键空间保持在"类别数量"这个小量级上。工匠/裁缝/炼金/厨艺
    /// 四个制作副职共用这一个变体，各自填不同的类别。
    ItemsCrafted(ContentIndex),
}
```

**`ItemsCrafted` 的计数键刻意不照抄 `kill_count_key`**：`crafting-system.md` 一节②核实到
`crates/ll-content/src/remap.rs:343` 明确不重映射 `script_state`，而 `kill_count_key` 把
`ContentIndex::get()` 的**数值**拼进键（`kill_count:37`）——mod 集合变化导致索引重编号后，
这些键会静默指向别的内容。`ItemsCrafted` 的键改用类别的 `NamespacedId` 字符串
（`craft_count:lostland:forging`），跨重编号稳定。**这条隐患是既有击杀计数就有的，
不是新引入的**，详见 `crafting-system.md` 八节。
...
（`ItemsCrafted(配方类别)`）与**同一笔投资**（`Intent::Craft` + `RecipeTable` +
`RecipeCategoryTable`）。订正后：

- **第 8 位「工匠/锻造」的理由作废**——不是「因果倒置、需要先造修理」，而是「与其余三个制作
  副职一起，阻塞在同一个正在设计的制作系统上」。升到第 3 梯队。
- **第 3 位「炼金/草药」的挂钩从 `ItemsPickedUp` 改成 `ItemsCrafted`**——更精确（熬药 vs 捡草药），
  且与其余三个共用同一份实现。原先为它单独规划的「拾取计数函数」不再必要。
- **第 5 位「厨艺」的『价值打折』作废**——类别闸门给了它一条本文档当初认为不存在的奖励通道
  （一节候选表「厨艺」行已订正）。
- **新增「裁缝」**（项目所有者 2026-08-22 点名）——与工匠共用全部机制，同第 3 梯队。

**1、2 两位不变**：测绘/制图（零新增存储）与殡葬/掘尸（挂载点已落地）仍然最便宜，
制作类四项需要先落地整个制作系统。但**一笔投资解锁四个方向**这一点，让制作系统在
「性价比」而非「绝对成本」上排得很靠前。
...
- `knowledge/design/crafting-system.md`——制作系统（配方类别、`Intent::Craft`、场地/工具前置、
  副职闸门）；本文档 2026-08-22 三处订正的来源，`SubclassUnlockTrigger::ItemsCrafted` 的完整论证
