...
> **【第三次订正，耐久扩面批次】那条「耐久适用范围」的裁定已经下来了，隐忧整个消失。**
> 所有者原话：「衣服要耐久，受到攻击就会减少耐久。」衣服因此**会坏**——裁缝与工匠现在处在
> 同一条重复需求循环上，下面第 1 条列的温度/保暖只是补法之一，不再是唯一的补法。
...
补给方式，它是一种**接敌方式**——`bb6cda8` 把它接进的是攻击判定（潜行中攻击更容易打出
偷袭；那一批写的是「直通」即必定触发，项目所有者后来裁定去掉那条「必定」，潜行改成偷袭
判定里的一个修正，见 `ll_sim::combat::STEALTH_SNEAK_MODIFIER`），不是任何补给循环。
...
### 【第三次订正 · 项目所有者裁决】「副职不给数值」这条被推翻了

**谁推翻的**：项目所有者。**原话**：

> 「那副职还是给点数好了。并且是带有技能的。」

**被推翻的是本节开头那句框架核心判据**——原文写的是「副职不给数值，只当『能不能学』的资格闸门；
给东西的是天赋」，三节「`SubclassDef` 的最小形状」里那条红线「**不加任何数值字段**」也随之只剩
一半有效。**原文一个字都没有删**（本仓库的订正纪律：留下被推翻的原文，读者才能看出判断是怎么变的），
下面逐条说明还剩什么、变了什么。

**为什么会被推翻**：本节原来的逻辑是「副职只是资格，一切效果经天赋兑现」，它自洽、也确实防住了
一之二⑨抓到的那次真实违反（把一个负重上限数字包装成副职）。但它有一个没被说破的代价——
**一个纯资格的副职在玩家那一侧是没有获得感的**：辛辛苦苦锻了二十次铁拿到「工匠」，角色面板上多一行
字，此外什么都没发生。项目所有者的裁决把这条代价明确定价为「不可接受」。

**裁决拆成两半，各自的落点完全不同**：

1. **「带有技能的」——已落地（副职天赋接线批次）。** 这一半**不违反**本节的红线，它落的正是本节
   第 2 条一直在说的那条通道：`SubclassDef` 新增 `traits: Vec<TraitGrant>`，副职自身仍然零存储
   数值，效果载荷全在 `TraitTable` 里。落地形状与本节【第二次订正】的预判逐字一致——给
   `SubclassTable` 补一个 `impl TraitGrantSource`，再往 `agent_trait_sources` 里加一路，
   返回类型从 `[TraitSource; 2]` 变成 `Vec<TraitSource>`（长度 `2 + 副职数`，上限由
   `MAX_SUBCLASSES` 封顶，见五节）。**代码里没有为副职新开任何一段聚合算法**（ADR 0021 复核结论记在
...
2. **「给点数」——已落地（副职发点批次），且它才是真正推翻红线的那一半。** 属性点/技能点是**纯数值资源**，
   ...
   不再成立。**未落地的原因不是难，是它有一个必须先答的问题**：

   > `Effect::GrantSubclass` 有上限（`MAX_SUBCLASSES = 3`）与放弃路径（`Intent::AbandonSubclass`）,
   > 而 `ll_sim::subclass::grant_subclass_effects` 的文档明确写着：满员时被拒绝的是授予、**不是
   > 计数**，玩家放弃一个副职腾出槽位后「下一次在该类别里制作时判据是 `累计次数 >= threshold`
   > （不是 `== threshold`），当场达标、当场授予」。**放弃再重新获得因此是一条真实可走的路径**——
   > 若获得副职时无条件发一批点数，它就是一台点数复制机。

   这与技能点批次立下的那条纪律（「退点会让『换 mod 读档再换回来』变成点数复制机」）是同一条。
...
- **一之二⑨的判定仍然成立**：「驮运」的全部内容是一个负重上限数字，它仍然不该是一个副职，仍然该是
  一条 `TraitGrant`。裁决说的是「副职可以给点数」，不是「任何给数值的东西都可以是副职」——
  一条内容够不够格当副职，判据仍在一之二「合并副职的正确判据」那一节。
- **三节「不把获得条件放在 `SubclassDef` 上」仍然成立**，且已落地为并存的两个字段
  （`unlock` 与 `traits`，见 `mods/example_mod/subclasses.json5`）。
- **「不需要独立的命名空间字段」仍然成立**——恰恰相反，正是主职副职共享技能命名空间（裁定 P5-4）
  让 `traits` 这条引用不需要任何跨号段机制。
- **本节第 1 条（`skill-requires!` 那条闸）仍然全部未落地**，不受本次裁决影响。
...
> **【订正，耐久扩面批次】⑩ 那条冲突已由所有者裁定解除并落地**：原话「修理锤子也算是一种
> 武器，也可以是带有功能性的物品。只要使用就会减少耐久。」`resolve_craft` 现在会在制作真的
> 发生时扣掉被点名工具的一点耐久，见 `crafting-system.md` 六节末尾的订正框。
> **⑦（技艺浮动）与本文档二节「副职不给数值」的冲突不受影响，仍然悬着。**
