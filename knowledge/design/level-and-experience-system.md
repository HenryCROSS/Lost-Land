...
**冻结于** 2026-08-20。**落地状态**：纯设计，`crates/` 中无任何对应类型（`level`/`experience`/`XpCurve` 均不存在，核实见一节）。核对提交 `98621f5`（`main` 分支）。
...
- **按死者自身的 `level` 计算经验**——否决（当前批次）：多数击杀的目标是薄层杂兵,没有 per-instance 等级字段可读,这条路只能覆盖厚层实体（玩家、随从、具名怪物），不是通用方案；若未来薄层也需要某种"等级"概念（例如给薄层加一列近似等级用于经验计算),是一次独立的、需要单独评估存储代价的设计,本文档不代为决定。
