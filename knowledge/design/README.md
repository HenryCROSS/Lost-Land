...
| [天赋/特性系统](trait-system.md) | `TraitDef`/`TraitTable` 独立内容类型（授予技能/属性修正/`RuleModifier` 规则修正三类效果的统一载荷）、天赋与 buff 共用载荷不共用实例化（天赋是引用零存储、buff 是 `ActiveTraitInstance` 真实偏差需进 `hash()`）、`RuleModifier`（抗性/重骰/优势劣势，声明式，减伤链路读取而非事件监听器）、`TraitGrant`（授予关系带 `unlock_level`，种族/副职/装备/buff 恒填 1）、亚种走 `RaceDef.parent_race`（不照抄副职）、`register-trait` 签名与一档判据、六个 D&D 示例（矮人抗毒/龙裔吐息/半身人幸运/野蛮人狂暴/盗贼偷袭/法师法术位）逐一核实能不能表达 | 抗性乘数机制与判定/检定系统本身（均不存在，只给挂载点）、法术位分级资源池与休息事件（明确排除，见文档七节，法师法术位因此无法表达）、等级/经验曲线本身（归另一份并行设计的等级系统文档，本文档只消费 `unlock_level` 这个字段）、P6 装备授予天赋（等 P6） |
