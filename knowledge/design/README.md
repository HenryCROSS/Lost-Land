...
| [社会系统](society-and-affiliation.md) | `Affiliation` 统一结构（**[2026-08-26 更正]** 原文写「势力/宗教/行会/文化/家族/职业**六类**」，`Profession` 变体已在文化批次删除，今天是**五类**：势力/宗教/行会/文化/家族）、`CultureDef` 生成权重、地图结构层（聚落/道路/遗迹/地标/资源点）、宗教如何运转、家族与代际、关系系统的默认派生与记忆偏移、性格 `Traits`、职业声望的局部化、LOD 兼容性 | 具体的悬赏任务结构、行会定价公式、钱包与货币守恒（见经济文档）、物品本身的字段（见物品文档）、种族对关系派生基线的贡献（见种族文档「种族偏见」一节）、`Affiliation.org` 的具体 ID 类型（见身份文档） |
...
| `Affiliation` / `AffiliationKind`（**[2026-08-26 更正]** 势力/宗教/行会/文化/家族**五类**统一结构，`Profession` 已删除） | [社会系统](society-and-affiliation.md) | [物品系统](item-system.md)（`Owner::Faction`，`Owner` 本身仍零实现）、[Agent 目标与经济](agent-goals-and-economy.md)（行会中介贸易即 `AffiliationKind::Guild`；原文「职业审计即 `AffiliationKind::Profession`」**已失效**——职业今天只由 `Agent.profession` 表达） |
| `CultureDef`（文化生成权重） | [社会系统](society-and-affiliation.md) | **[2026-08-26 更正]** 已落地，但**代码里不叫 `CultureDef`**——走 `TerrainKind`/`TerrainAttrs`/`TerrainTable` 的既有形状，拆成 `CultureKind`/`CultureAttrs`/`CultureTable` 三个类型（`crates/ll-world/src/culture.rs`），数据由 `ll-mod` 从 `mods/lostland/cultures.json5` 装载后注入世界生成。六个设计字段落地四个（建材只有墙、经济落成单值），`naming`/`social_structure`/`religion_affinity` 仍未落地 |
...
| `Agent.race: ContentIndex`（薄层已无 `race` 列） | [种族系统](race-system.md) | `Agent.race` 已落地为 `crates/ll-world/src/entity/agent.rs`。**[2026-08-26 更正]** 原文「薄层落地方式与设计不符——当前是显式存储列」**已不再成立**：文化批次（提交 `4aec07e`）已删除 `ThinPopulation` 的 `race` 列，种族改由「出生聚落 + 文化的建立者种族权重」现算派生（`promote()` 由调用方递 `race` 进来），正是种族文档「存储」一节原本的设计，判据是 ADR [0009](../decisions/0009-derive-by-default-store-only-deviation.md)「能派生的不进存档」。该文档的实现债务框已结案 |
| `birth_settlement`（出生地，终身不变，区别于会随迁徙变化的 `settlement`） | [种族系统](race-system.md)「存储」一节首次提出 | [命名、改名与本地化](naming-and-localization.md)（同一列同时驱动命名依据的文化查询）。**[2026-08-26 更正]** 不再是「未落地」，而是**被认定为与 `settlement` 同一列**——文化批次的推理是「`settlement` 那一列本来就是出生聚落，`birth_settlement` 不需要新增」（`crates/ll-world/src/entity/thin.rs:54`）。**但种族文档八节把两者设计成不同的东西**（一个终身不变、一个随迁徙变化），今天没有迁徙系统所以等号成立，迁徙落地时会断——已列为待所有者裁定项，见 [P6/P7/P8 阶段清算](../audit/2026-08-26-phase-reckoning-p6-p8.md) 七节 |
| `WorldId` / `OrgInstance`（世界生成实例的持久标识，区别于 mod 定义类型用的 `ContentIndex`） | [身份与 ID 空间](identity-and-ids.md) | [社会系统](society-and-affiliation.md)（**[2026-08-26 更正]** 原文「`Affiliation.org` 字段类型需要从 `ContentIndex` 改为 `WorldId`，尚未实现」**已经实现**，落成 `OrgRef` 枚举：`Def(ContentIndex)` / `Instance(WorldId)`，`crates/ll-world/src/entity/affiliation.rs`）、[世界历史生成](world-history.md)（势力/家族/聚落的产出方）。**`OrgInstance` 类型已落地**（`crates/ll-world/src/entity/org.rs:21`）**但全仓库零生产构造点**，是一处教科书式的「声明了但从没接线」 |
...
| `NamingRules`（命名规则：音素表 + 拼接方式） | [社会系统](society-and-affiliation.md) 提出字段位置（`CultureDef.naming`）；真正的类型定义与实现在 `crates/ll-world/src/naming.rs`（类型已落地，**但 [2026-08-26 核实] 唯一消费者仍是 `crates/ll-sim/examples/p3_acceptance` 这个 demo，生产路径零调用——NPC 至今没有名字，`Agent` 连 `name` 字段都没有**；文化批次落地的 `CultureAttrs` 也没有 `naming` 字段） | [命名、改名与本地化](naming-and-localization.md)（多语言版本表长对齐规则、命名依据出生地文化而非种族） |
...
7. **种族**——`race-system.md`「存储」一节：`race(entity) = weighted_pick(settlement_race_weights[birth_settlement], hash(...))`，理想情况下薄层零列现算。**[2026-08-26 更正]** 原文「但当前代码已把 `race` 做成显式存储列，与这条设计冲突，是一处待对齐的实现债务」**已不再成立**：文化批次已删除该列，实现与设计对齐，实现债务框已结案。
...
[界面布局与导航模型](ui-and-navigation.md) 与前面任何一条依赖链都不同——**它是这批文档里唯一一份盘点「已经跑在玩家机器上的东西」的**，前五节全是现状清单，每一条都带 `文件:行号`，后六节才是规格。建议先读「零、诊断」那一节：它给出的结论（今天有三套互不知情的模态系统）是后面所有条目的共同根因，不读它的话一至五节会读成一堆互不相干的小毛病。读之前建议先看过[行动能力与输入上下文](action-capability-and-input-context.md)（`UiMode` 栈与 `InputContext` 的原始裁定，本文档在「相关文档」一节逐条列出了那份文档现已过期的五处）与 [ADR 0002](../decisions/0002-integer-only-world-state.md)（本文档 6.1 节纠正了一条广为流传的误读：整数纪律管的是世界状态，UI 层用 `f32` 是 ADR 明文允许的）。八节「i18n 对布局的约束」有一条反直觉的实测结论值得单独看——**这个字体下英文比中文宽 44%**，所以「给翻译留余量」那条通行直觉在这里是反的，十条实测溢出里有八条只在英文构建下看得见。**【2026-08-31】P0 十二条已全部落地**（数据安全批、导航收敛批、布局与文本批），八节那张溢出清单下方追加了改后的实测值，九节 F1 的裁定有一处按所有者原话的更正；P1/P2 仍未做。
...
| 界面布局与导航模型 | **不是纯设计**——一至五节是对已落地界面的盘点（25 块界面、9 处跳转死路、8 处输入约定不一致、10 处 i18n 溢出，逐条带 `文件:行号`），六至十节是规格（P0 12 条 / P1 16 条 / P2 8 条），尚未落地任何一条；同批次只落地了存档文件名那一处代码改动（`crates/ll-game/src/save_slot.rs` 的 `SlotId::for_new_slot`） |
