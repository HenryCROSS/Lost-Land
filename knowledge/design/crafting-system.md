...
> **【订正，耐久扩面批次】三条路现在全部走通，上面这处不对称已经不存在。**
> 项目所有者推翻了「只有装备武器才有耐久」的裁定，新的两条原话：
>
> > 「衣服要耐久，受到攻击就会减少耐久。」>
> > 「修理锤子也算是一种武器，也可以是带有功能性的物品。只要使用就会减少耐久。」
>
> （a）**已落地**：耐久范围扩到「武器 + 护甲/衣物 + 工具」，注册期「只有占武器槽位的物品
> 才允许携带耐久」那条校验已删除；`resolve_attack` 新增「挨打」通道，防御方每一件落在非武器
> 槽位的已装备物品挨一下各掉一点耐久。衣服因此**会坏**，裁缝的产出不再是一次性买卖。
> （b）已落地（多来源抗性批次）。（c）已落地（`c12c04f`，`StatTarget::Insulation`）。
> 三条合起来：裁缝现在同时拥有重复需求（衣服会磨损）与两条独立的加成轴（抗性、保暖）。
...
> **【订正，耐久扩面批次】这个口子已由所有者裁定并落地：工具现在会因为制作而磨损。**
> 原话：「修理锤子也算是一种武器，也可以是带有功能性的物品。**只要使用就会减少耐久**。」
> `resolve_craft` 在第 9 步（全部前置与食材校验之后，即**制作真的发生时**）产出一条
> `Effect::AdjustEquipmentDurability`，让被配方点名的工具损失
> `TOOL_DURABILITY_LOSS_PER_CRAFT`（1）点耐久；工具堆没有耐久概念（`durability == None`）
> 时不产出任何效果。磨到零之后本节上一段那条既有谓词照常把制作挡下来——工具会用坏、用坏了
> 就不能用，闭环成立。
...
| ⑩ **工具因制作而磨损** | ~~与所有者「只有装备武器才有耐久」的裁定直接冲突。六节末尾已标注为需所有者定夺的口子，本文档不擅自扩大耐久范围~~ **【订正，耐久扩面批次：所有者已裁定「只要使用就会减少耐久」，本条已落地，见六节末尾的订正框】** |
...
**项目所有者选择了「推翻五节」这一支**，原话：「科研可以通过加点解锁，最开始设有初始可以
通过阅读获取经验，也或者通过研究其他物品获取经验。**菜谱就是通过随机丢入东西煮获取或者
阅读书籍的时候获取。**」——不是「把科研限定在非食物类别上」，恰恰相反：菜谱被点名为要靠
发现获取的那一类。`food-and-cooking-system.md` 五节末尾已补上更正记录（原文未删）。

**上表右列「配方解锁：不做」那一格因此改成「做」**，其余一字未改：

| | 是什么 | 现状 |
|---|---|---|
| **类别访问权**（七节） | 你是不是工匠 | **已做**。读 `Agent.subclasses`，零新增字段 |
| **配方解锁** | 你知不知道这张图纸 | **已做**（配方发现批次）。`Agent.known_recipes` + `Effect::LearnRecipe` |

**本文档定形的部分确实一处未动**，那句预判兑现了：`resolve_craft` 的判定顺序只是在副职闸门
之后插进第 4 道「已知闸门」，三、四、六、七节定下的 `RecipeDef`/`RecipeCategoryDef` 形状、
类别表达方式、场地/工具判定、副职闸门全部原样保留。新增的只有两条各自独立的字段
（`RecipeDef.requires_discovery`、`ItemDef.taught_recipes`）与两个新意图
（`Intent::Read`、`Intent::Experiment`）。

**八节 E 那条计数键纪律在本批次继续生效**：新增的两条发现路径都不写任何计数键——读书与试做
都不经过脚本状态存储（`known_recipes` 是 `WorldState` 的一等字段，走
`Effect::LearnRecipe` → `apply`，ADR 0023），因此不存在照抄 `kill_count_key` 数值键的机会。

#### 后续裁决：研究经验被**收窄**成两条来源（未鉴定物品批次）

上面那条原话里的「通过阅读获取经验，也或者**通过研究其他物品**获取经验」留下了一个缺口：
非书籍物品没有「教配方」那样天然的一次性事件，防刷因此需要一份新的逐实体「研究过没有」
状态——而那正是本仓库一贯要避开的东西。

**项目所有者裁定用「未鉴定物品」补上那个缺口，并就此收窄经验来源**，原话：

> 「我觉得可以**加入未鉴定物品，通过鉴定获取属性和说明，同时就能获得经验**。然后就
> **收窄成通过未鉴定物品和书籍获取经验**就好了。」

落地形状（未鉴定物品批次）：

| 来源 | 一次性事件是什么 | 闸门读哪个字段 |
|---|---|---|
| **鉴定** | 认出一个**此前不认得的物品种类** | `Agent.identified_items` |
| **读书** | 真的教到一条**此前不知道的配方** | `Agent.known_recipes` |

两条闸门读的都是**它们各自的产出目标**，因此「只有真的学到新东西才给经验」是结构性的，
**零新增逐实体状态**——这正是这条设计最值钱的性质。重复鉴定同一种、重读读透了的书都恒零
收益，连时间都不推进。

**乱煮（`Intent::Experiment`）不给经验**：收窄的直接后果，它仍然只教配方。

新增的内容字段是 `ItemDef.requires_identification` 与 `ItemDef.study_experience`
（两条路径**共用**后者，理由见该字段文档）；新增的意图/效果是 `Intent::Identify` 与
`Effect::IdentifyItem`。经验走的是**既有那条**数轴（`Effect::GrantExperience` + `xp_curve`），
**没有**另开一条「科研经验」——那需要复制整台升级级联机器，正是 ADR 0021 点名要避免的抽象。

#### 后续裁决：盲盒（盲盒批次）

**项目所有者原话**：

> 「我希望能加入**盲盒**这种物品，**鉴定了可以获取经验，同时会随机获得一件物品或者武器
> 装备**。」

盲盒复用 `Intent::Identify` 这一个动作，但语义与普通鉴定**本质不同**：

| | 普通鉴定 | 盲盒 |
|---|---|---|
| 物品去向 | **留着**（只是你现在认得它了） | **被消耗** |
| 产出 | 无 | **一件随机物品** |
| 性质 | 揭示 | **转化** |

池子按权重声明（`ItemDef.blind_box_pool`），选取算法照抄
`ll_world::weather::weather_kind_at` 那套「权重求和 → `DetRng::gen_range` → 前缀和 walk」，
不另发明；随机走 `DetRng::for_entity` 三元组流，**没有盲盒声明时不构造流**（同
`resolve_attack` 对偷袭的既有纪律）。

**⚠ 盲盒是「只有学到新东西才给经验」那条原则的有意例外。** 所有者裁定，原话：
**「开盲盒都给吧，轻松点，这是游戏。」** 每开一个盒子给一次经验，不查产出物认不认识、也
不查这种盒子开过没有。取舍的两条前提：

1. 那条原则**仍然完整适用于普通鉴定与读书**，一个字没改；
2. 经验产出的上限由「世界里有多少盒子」决定，不由玩家按了几次决定——盒子目前是纯粹的世界
   产物，**没有配方产出它，也没有交易系统能买到它**。

**第 2 条是有条件的**：哪天有人给盲盒写了配方（或让它可购买），这个上限当场消失，盲盒会
变成经验水龙头。那时需要**重新裁定**，而不是默默加一条配方。这句警告同时写在
`ll_mod::item::ItemDef::blind_box_pool` 的字段文档与 `mods/lostland/items.json5` 那件盒子的
注释里，让加配方的人在写下 `product: "…blind_box"` 之前就看见它。
...
零新增 `Effect`、`ConsumeInventoryItem` 不加 `amount`）**本文档全部沿用，一条未改**。

**唯一的例外是「菜谱不设解锁门槛」那一条**——它在本文档写作时确实被沿用，此后被项目所有者
推翻，见上面十四节①的裁决结果一段。
