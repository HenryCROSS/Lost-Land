...
> **【2026-08-31 已裁定并落地：走（a），且它不再是「纯派生不进存档」】**
>
> 所有者原话把这条待裁定关掉了：「建筑需要根据他的类型填入不同的家具，
> 例如箱子，椅子，床，书柜等」「每个物品都会有个主人，一个建筑内的物品
> 通常都是属于某个人的」。落地批次是
> `docs/superpowers/plans/2026-08-31-batch20-buildings.md`。
>
> **上面那句「（a）……纯派生、按 ADR 0009 不进存档」是错的，原文保留、
> 在这里更正**：家具是 `WorldState::ground_items` 的条目，而那个字段
> **随存档主体一起走**（`ll_world::state::WorldState` 的 `ground_items`
> 字段进 `hash()`、进 postcard 主体）。它做不到「不进存档」，也不该做到
> ——玩家搬走一把椅子之后，那把椅子在哪儿是**偏差**，正是 ADR 0009 要
> 存的那一半。
>
> 落地形状与（a）的另一处不同：**摆家具的时机不是 `stamp_settlement`
> 铺建筑那一刻**，而是**据点物化那一趟**（`ll_game::settlement_spawn`
> 的 `furnish_settlement`，紧跟在 NPC 物化之后）。理由是
> `stamp_settlement` 只拿得到一个 `ChunkGrid`，拿不到 `WorldState`；
> 而按据点触发还顺带解决了「一座据点横跨最多 8 个区块、按区块触发会被
> 触发多次」那个问题——与 NPC 物化逐字同一条理由。
>
> **家具的主人**：`Owner::Faction(SettlementSite::id)`，在构造
> `ItemStack` 的那一刻就带上，不事后回填。这是
> `ll_world::ownership::Owner::Faction` 文档里那条裁定的第一个构造点。
>
> **建筑类型**由文化在 `cultures.json5` 里声明
> （`CultureAttrs::buildings`，见 `ll_world::building`）——本体出现四类：
> 住宅 / 作坊 / 仓库 / 酒馆。
>
> **（b）「NPC 自己造」仍然是将来的路**，两条并存不冲突，上面那句
> 「引擎侧不阻碍任何一条」原样成立。
