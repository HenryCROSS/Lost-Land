...
**落地状态**（2026-08-19 复核更正——原文「纯设计」已部分过期）：**部分落地**。任务 2「`ClassDef` 职业注册表」（提交 `580a7bd`）与任务 3「`SkillDef` 技能注册表——前置关系 DAG 校验」（提交 `c10a3e9`）均已完成，代码见 `crates/ll-mod/src/class.rs`/`crates/ll-mod/src/skill.rs`，走本文档「与既有架构的接线点」一节描述的 `ClassTable`/`SkillTable` 注册表模式，「本体即 Mod」检验（本体注册函数与 mod 注册函数是同一个公开函数）已随任务落地。`SubclassDef`（任务 4）与 `QuestDef`（任务 6）**仍未落地**，本文档只定了这两者的字段形状，实现留给各自任务——本文档最初「本文档只定四个 Def 的字段形状……不设计任何具体游戏内容」这句概述现在只对 `SubclassDef`/`QuestDef` 两个仍成立，对已落地的 `ClassDef`/`SkillDef` 已经不准确，读者应以 `crates/ll-mod/src/{class,skill}.rs` 为准。
...
pub struct QuestNodeDef {
    ...
    /// （ADR 0016/0017 的三档分级同样适用于任务系统,但本计划不做二档
