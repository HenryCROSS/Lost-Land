...
> **落地后的更正二（加值类型批次）**：本小节两处内容被项目所有者的后续裁定取代，逐条列明。
>
> **一、抗性不再是乘数，是减伤点数。** 本小节通篇写的 `resistance_multiplier(defender, damage_category)` 与"取乘数"已经作废：所有者裁定**全部规则修正改成整数点数**，抗性因此改成 flat DR（`ll_sim::rule_modifier::resistance_damage_reduction` 返回**减掉多少点**，缺省 0）。完整论证与"哪些量改、哪些量不改"的分界线见 `damage-formula-mod-api.md` 二十节末尾三段更正。同一次改型下，本文档同节的两条盘查被动也从乘数改成点数（`InspectionSuspicion` 现在是从盘查触发概率上直接减掉的千分比点数）。
>
> **二、"取第一条 / 取最强的一条"扩成两级：同一类型取最强，不同类型相加。** 所有者引入 D&D 3.5e / 开拓者的**加值类型**模型：
>
> ```text
> 同一类型 → 取最强，不叠加      ← 上面那条 tie-break，一个字没改
> 不同类型 → 相加
> ```
>
> 类型是一张开放注册表（`ll_mod::modifier_type`，与 `damage_category`/`weapon_category` 同一套手法），mod 可以声明自己的类型；引用未注册的类型在装载期当场报错。**不声明类型的全部落进同一个共享桶**——这条默认值是刻意的，3.5e 里"无类型"加值永远叠加，照搬会把现有的每一条声明静默地从"取最强"改成"全部叠加"。分桶层因此在一份没有任何类型声明的内容上是恒等变换，由 `ll_sim::rule_modifier` 的单元测试 `不声明类型时结算结果与分桶之前逐位相同` 钉住。
>
> **本小节原文"不取乘积，理由是『免疫 500‰ 又免疫一次』不应该变成 25% 而不是 0%"这半句仍然成立**，只是在点数模型下的表述是"同一类型不相加"：两条同类型的 4 点减伤还是 4 点。
>
> **一之二、脆弱另立一个变体，不再是负的减伤（易伤与减伤对称批次）。** 更正一落地时脆弱是用 `damage_reduction: -5` 表达的，那条写法**已撤销**：桶内"取最强"的方向声明成"减伤点数越大越强"，于是同一个桶里 `+3` 会把 `-5` 静默吃掉，而"不声明加值类型的全部落进同一个共享桶"正是本项目刻意选的默认值。脆弱现在是 `ll_sim::rule_modifier::RuleModifier::Vulnerability`（`damage_increase`，恒非负），与抗性**各自聚合**、最后一减一加；`Resistance.damage_reduction` 同批次收窄成恒非负。完整论证见 `damage-formula-mod-api.md` 二十节「落地后的更正四」。
>
> **三、"免疫"不再可声明。** 减伤不封顶（大伤害自然穿透），但一次本来打得出伤害的攻击减完至少还剩 1 点（`MINIMUM_DAMAGE_AFTER_RESISTANCE`）——所有者裁定"不允许绝对"。这条**推翻**了 `damage-formula-mod-api.md` 二十节原本"免疫理应打出 0 点"的论证，更正段写在该节末尾。两条盘查被动那一侧的对应处理是概率**两端各留一线**（`PROBABILITY_MARGIN_PERMILLE`，1‰）：永远不会必定成功、也永远不会必定失败。
>
> **落地追记（判定系统批次）：** 上一句里的 `PROBABILITY_MARGIN_PERMILLE` 与 `clamp_probability_permille` **已经不存在了**——两条盘查被动改走对抗判定之后，那两个 pub 项的最后一个消费者（卫兵盘查、逐件藏匿）随之消失，本批次一并删除。所有者「不允许绝对」那条裁定在这一侧没有被放弃，只是换了守法：现在由 `ll_sim::check::CheckDice::max_modifier` 推出的修正上限 `L = (M(N−1)−1)/2` 保证（装载期就拒掉超过 L 的声明，运行期聚合后再钳一次），两侧的 `MdN` 因此永远留得下至少一格重叠。`MINIMUM_DAMAGE_AFTER_RESISTANCE` 那一半原样成立。
...
> **落地追记（判定系统批次，`ll_sim::check`）：** 这条预言兑现了，`RuleModifier::RerollOnce` 确实一个字段都没改。但兑现的方式与预测**不同**，这一点要如实记下：
>
> - 预测的路径是"先给伤害公式求值器补 `roll_one_die` 身份钩子，再扩展到判定求值器"。实际路径是**跳过了前半截**——`roll_one_die` 至今没有落地，重掷直接接在判定的掷骰原语（`CheckDice::roll_once`）上。
> - 事后看，"下沉到伤害公式任意骰子取值"这个范围收窄本身就是将就：`RerollOnce` 的语义是"这个人掷判定骰的手气"，它**本来就属于判定层**。等到判定层真的存在，它自然落回原位，那个钩子也就不再是它的前置。
> - 与此同时，判定用的是 **MdN**（`M`/`N` 都是参数，引擎当前取 `3d20`），不是写死的 d20。半身人幸运因此覆盖的是"任意一颗判定骰"，比 5e 原版的"d20"更宽——`N` 换了它仍然成立。
> - 本文档下方"等什么"第 4 条（`roll_one_die` 钩子）**因此不再是 `RerollOnce` 的阻塞项**；它若还要做，理由得换成伤害公式自己的需求。
...
5. ~~**判定/检定系统不存在**——`RuleModifier::Advantage`/`Disadvantage`、半身人幸运针对 d20 的真正语义、盗贼偷袭"有优势"这半个条件，全部等待 `attribute-system.md` 八节的 d20 判定机制真正接入战斗（当前只是纯设计,从未接线）。~~
   **已落地（判定系统批次）**：`ll_sim::check` 的对抗判定（`主动方 MdN + 修正 vs 被动方 MdN + 修正`），`Advantage`/`Disadvantage`/`RerollOnce` 三个变体全部接上消费者（`ll_sim::rule_modifier::check_roll_bias`/`check_reroll_value`），`scripts/ci/check_field_consumers.py` 里那三条豁免已删除。
   两处**与本条原文不同**的落点：
   - 判定不是写死的 d20，是 **MdN**（项目所有者裁定"不一定是 20，可以是 N，正整数""并且可以一次性投掷 M 个骰子"）。`M >= 2` 给出钟形分布，修正在势均力敌时影响大、在悬殊时影响小——这是设计旋钮，不是副作用。
   - 修正上限**必须相对于 `M` 和 `N`**：`L = (M×(N−1) − 1) / 2`。d20 传统的"±1..±10"正是 `1d20` 代进这个式子的结果（`L = 9`）；`N` 可配之后照抄绝对数字会让 `N = 1000` 时一条 `+400` 压垮一切。
   仍未落地的只剩**盗贼偷袭"目标附近有盟友"**那半个条件——它卡的是下一条（`resolve` 没有战场空间上下文），与判定系统无关。
