...
- [2026-08-26 P6/P7/P8 阶段清算](audit/2026-08-26-phase-reckoning-p6-p8.md) — 补上 P6/P7/P8 从未被记录过的阶段账：三个阶段各自在规格里写了什么、代码里真正落地了多少、哪些被静默跳过；给出「当前处于哪一个 P」的判定与下一批入口，并逐条复核了历轮悬空债务（含三条已不成立的纠正）
- [2026-08-29 文档—代码一致性审计与未接线声明盘点](audit/2026-08-29-doc-code-audit.md) — **设计文档总索引「落地状态速览」27 行里 20 行是错的**；「更正写在更正方、不写在被更正方」这个反复出现的形状；12 项未接线声明逐条判成缺陷（5）还是刻意搁置（7）

### 阶段交接

> **【2026-08-30：这一节此前只列了八份 `pN-to-pN+1.md`，两份会话交接一条都没有。】**
> 而那两份才是承载**所有者十三条裁定**、**当前待办**与**已知缺陷**的那两份——从这份
> README 出发的人以前根本找不到它们。已补在最前，见
> [2026-08-29 文档—代码一致性审计](audit/2026-08-29-doc-code-audit.md) 三节第 4 条。

**两种切分方式，管的东西不同，不要只读一种**：

- **按会话切分**（`YYYY-MM-DD-session-handoff.md`）——记的是**会话内**的进度、
  新落地了什么、待所有者裁定的是什么、已知未修的缺陷是什么。**阶段边界切不出这些**。
- **按阶段切分**（`pN-to-pN+1.md`）——记的是一个阶段留给下一个阶段的接口债务与前瞻判断。
  注意 `p7-to-p8.md` 是**预写**的（P7 尚未收尾），它记不了 P7 内部的进度。

**新接手的人从这里开始读**：

1. [会话交接 2026-08-28](handoff/2026-08-28-session-handoff.md) — **先读整份**。
   第〇节「不要在文档里找三个关键常量的值」（跑 grep，别抄）、第〇之二节
   **所有者 2026-08-29 一次性裁定的十三条**、第四节待裁定、第五节已知缺陷。
2. [会话交接 2026-08-27](handoff/2026-08-27-session-handoff.md) — **第一节六条纪律
   全部仍然有效**，08-28 那份在其上新增~~两条~~**三条**（第 9 条「更正必须写回
   被更正方」是 2026-08-30 补进去的），不重复。
3. [2026-08-29 文档—代码一致性审计](audit/2026-08-29-doc-code-audit.md) — 哪些文档
   今天与代码对不上、哪些「未接线」是缺陷哪些是刻意搁置。**开工前扫一眼，能省一次白做工。**
4. 再按需读下面对应阶段的 `pN-to-pN+1.md`。

- [会话交接 2026-08-28](handoff/2026-08-28-session-handoff.md) — 所有者十三条裁定、势力播种提前、气候条带与两条黄金基准重冻、三次「无法复现」的失败
- [会话交接 2026-08-27](handoff/2026-08-27-session-handoff.md) — 六条纪律、文化与战争、玩家第一次真的能玩、一个确定性系统里的硬死循环
...
- [P6 → P7 交接清单](handoff/p6-to-p7.md) — 补写：P6 交付了什么、`Owner` 为何被显式拒绝而拒绝前提又如何变化、任务击杀进度与其余三条击杀路径的字段分叉、11 个无生产者 `Intent` 的分组
- [P7 → P8 交接清单](handoff/p7-to-p8.md) — 预写（P7 尚未收尾）：P7 还差的四项、生成期 mod 集合被每次存档静默覆盖、P8 三件事全部踩在一套没有生产者的归属体系上
