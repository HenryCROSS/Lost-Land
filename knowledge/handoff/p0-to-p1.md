...
- **Linux Wayland 下窗口标题栏不显示标题文字**。这是 `wayland-csd-adwaita-notitle` 的代价，为移除已停止维护的 `ttf-parser` 而付出。装饰边框仍在。P7 接入 i18n 后此现象依旧存在（[2026-08-18 规格修订] 插入「物品与装备」新 P6 阶段后 i18n/UI 层原 P6 顺移为 P7），**不是缺陷**。
