...
/// 由首次运行记录的黄金基准。
///
/// # 本次重冻的原因（P2 最终评审 Important 4）
///
/// `Tick::is_daylight()` 从固定的 `6..18` 小时边界改为直接由
/// `ll_core::light::day_curve` 是否达到阈值派生（详见 `time.rs` 与
/// `light.rs` 的模块文档：收敛两套互相矛盾的白昼定义为同一份）。这条
/// 摘要按 `day % 24` 采样了全部 24 个小时，其中 18 点这一采样点的
/// `is_daylight()` 结果由 `false` 变为 `true`——曲线在 18 点整恰好等于
/// 判定阈值，新定义在这一点判定为白昼，旧的固定边界不是。这是算法
/// 本身的有意变更，不是平台相关行为泄漏，期望值随之更新。
const EXPECTED_TIME_DIGEST: u64 = 16_025_319_418_161_907_204;
