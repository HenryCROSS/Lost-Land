...
//! 跨平台逐位一致。
//!
//! 严格地说这里用的是**迭代变体**：规范做法是每次给状态加固定增量再混合，
//! 周期恒为 2^64；此处直接在混合函数上迭代，周期取决于起点所在的环。
//! 由于每个 `(实体, 事件)` 三元组只取少量数值，这个差别没有实际影响，
//! 但换算法或延长单条流的取数量之前应重新评估。
...
impl DetRng {
    ...
    ///
    /// 采用 Lemire 乘法取余的**快路径**：刻意省略了拒绝采样步，
    /// 残余偏差不超过 `n / 2^64`，实践中不可测量。相比之下朴素的取余
    /// 会让较小的值出现概率显著偏高，在百万次经济模拟中累积成系统性倾斜。
    ///
    /// **不要事后补上拒绝步**：那会改变取值流，使全部黄金基准作废。
