...
//!
//! 〔2026-09-02，批次 34〕上面那句「不声明就是缺一条，照样红」**只在
//! 一条规则都匹配不到时成立**，实测有一个洞：`dialogue-steward-root`
//! 的参数化规则删掉之后，更粗的 `dialogue-` 那条照样匹配得到，于是它
//! 静悄悄地退回按**模式串**量行数，本门禁**不红**。补上的那一条是
//! [`带变量的键必须归参数化`]——判据是通用的（文案里有 Fluent 变量的
//! 键一律不许走 `行数上限`），不点名任何一条键，因此下一个人给任意一条
//! 键加变量而忘了改分类，它也咬得住。
...
fn 分类表() -> Vec<分类规则> {
    vec![
        ...
        // 交易屏的两条行文案（批次 31，对话系统的批次 5）：物品名、
        // 数量、价钱三样都是运行期插值，量不出静态宽度。**必须排在
        // 通配的 `screen-` 那条之前被命中**——本表按最长前缀优先，
        // 这两条前缀更长，天然赢；其余三条（title/empty/hint）落通配的
        // 「模态屏行」一档。
        分类规则 {
            前缀: "screen-trade-buy",
            面板: "交易屏",
            判据: || 宽度判据::参数化("物品名、数量与价钱都是运行期插值"),
        },
        分类规则 {
            前缀: "screen-trade-sell",
            面板: "交易屏",
            判据: || 宽度判据::参数化("物品名、数量与价钱都是运行期插值"),
        },
        分类规则 {
        ...
        // 宽度，改这一条规则即可。
        //
        // 〔2026-08-31，批次 29〕上面那句「最长的是
        // `dialogue-steward-reward`」**已经过期**：那一句同批被改写了
        // （东西当场交到手上，不再是「等库房清点完」），另外新增了两行
        // 更短的选项文案。重新实测的余量：把这条规则的内容宽从 500px 收到
        // **470px 时全部会话屏文案仍然绿**，收到 380px 才第一次红
        // （`en / dialogue-steward-duties`，3 行 / 预算 2 行，最长行
        // 374.9px）。**预算仍然是全表最紧的几条之一，新增台词照旧要先量
        // 再写；红了改文案，不放宽这条规则。**
        分类规则 {
            前缀: "dialogue-",
            面板: "会话屏",
        ...
        // 〔2026-09-02，批次 34（NPC 姓名，对话批次 6）〕两条开场白里的
        // 职业名换成了 `{ $npc_name }`，说话人的名字是渲染期按文化派生
        // 出来的。拿模式串本身去量量到的是占位符的宽度，与真实渲染宽没有
        // 关系——与 `hud-key-hint-` 完全同一种处境，因此同样归「参数化」。
        //
        // **分类表是最长前缀优先**，所以这两条更细的规则在上面那条
        // `dialogue-` 旁边写就行，顺序不影响结果。
        //
        // 拼好的整行由 `ll-game` 那一侧的
        // `会话屏标题在最长的名字下也排得进两行` 覆盖，而且它比取样更强：
        // 它从真实 `mods/lostland/cultures.json5` 的音素表算出**每份文化
        // 在每种语言下最长的可能名字**（音节数取上界、每个位置取最长
        // 音素），代进这两条台词真排一次。
        分类规则 {
            前缀: "dialogue-steward-root",
            面板: "会话屏（标题，带参数）",
            判据: || 宽度判据::参数化("说话人的名字由文化在渲染期派生"),
        },
        分类规则 {
            前缀: "dialogue-guard-root",
            面板: "会话屏（标题，带参数）",
            判据: || 宽度判据::参数化("说话人的名字由文化在渲染期派生"),
        },
        // ── 常驻 HUD ────────────────────────────────────────
        ...
        // 世界层底部那一行常驻按键提示（批次 23，规格 F6）。归「参数化」
        // 而不是量行数：模板里的 `{ $inventory }` 之类占位符会被换成
        // **玩家当前绑的键名**（`KeyI`/`F7`/……），量模板本身量出来的是
        // 占位符的宽度，与真实渲染宽没有关系。拼好的整行的宽度由
        // `ll_game::key_hint` 那条断言覆盖——与 `screen-savelist-row`
        // 同一个分工。
        分类规则 {
            前缀: "hud-key-hint-",
            面板: "按键提示行",
            判据: || 宽度判据::参数化("键名由玩家当前的键位表运行期插值"),
        },
        分类规则 {
            前缀: "hud-feedback-",
            面板: "反馈行",
...
/// 某种语言下**每一个顶层消息的原文**（`key = value` 里的 `value`）。
///
/// 与 [`全部键`] 读的是同一份文件、同一条「顶格 `key = value`」判据，
/// 但要的是**等号右边**——[`带变量的键必须归参数化`] 的判据是「这条
/// 文案里有没有 Fluent 变量」，那只能从原文看得出来，`Catalog::resolve`
/// 解析完就看不见了（缺参数时 Fluent 会把 `{$x}` 原样渲染，靠解析后的
/// 结果去判会把「渲染失败」与「本来就有变量」混成一件事）。
fn 全部文案(language: &str) -> Vec<(String, String)> {
    let path = Path::new(LOCALES).join(format!("{language}.ftl"));
    let text = std::fs::read_to_string(&path).expect("本体的 .ftl 必然读得到");
    text.lines()
        .filter_map(|line| {
...
#[test]
fn 带变量的键必须归参数化() {
    // # 这条是补上来的，起因是一次「改坏了它不红」（批次 34 实测）
    //
    // NPC 姓名批次给 `dialogue-steward-root` / `dialogue-guard-root` 两条
    // 键加了 `{ $npc_name }`，并把它们改归 `宽度判据::参数化`。
    // 按 ADR 0022 去验那条改动：**把那两条规则删掉，本门禁应当红**。
    // 实测**它没红**——因为更粗的 `dialogue-` 那条规则照样匹配得到，
    // 「缺一条就红」只在**一条规则都匹配不到**时才生效。于是那两条键
    // 会静悄悄地退回按**模式串**量行数，而模式串的宽度与真实渲染宽没有
    // 任何关系（`{ $npc_name }` 那 14 个字符既不是名字、也不是空）。
    //
    // 这条测试把那个洞补上，判据是**通用的**、不点名任何一条键：
    // **文案里带 Fluent 变量的键，一律不许走 `行数上限`。**
    // 它同时也是「判据适用面被新代码绕过」那类假绿的钉子——下一个人给
    // 任何一条键加变量而忘了改分类，这里当场红。
    //
    // 真实宽度由各自那条对「拼好的整行」的断言覆盖：
    // `hud-key-hint-` → `ll_game::key_hint`；
    // `dialogue-*-root` → `ll-game` 的 `会话屏标题在最长的名字下也排得进两行`。
    // Arrange
    let 表 = 分类表();

    // Act
    let mut 漏网: Vec<String> = Vec::new();
    let mut 查过的带变量的键 = 0_usize;
    for language in LANGUAGES {
        for (key, 文案) in 全部文案(language) {
            if !文案.contains("{ $") && !文案.contains("{$") {
                continue;
            }
            查过的带变量的键 += 1;
            let rule = 命中规则(&表, &key).expect("每个键都有规则，见上一条测试");
            if matches!((rule.判据)(), 宽度判据::行数上限 { .. }) {
                漏网.push(format!(
                    "{language}: {key}（命中规则前缀 {:?}，面板 {}）",
                    rule.前缀, rule.面板
                ));
            }
        }
    ...
    assert!(
        漏网.is_empty(),
        "这些键的文案里有 Fluent 变量，却在按模式串量行数——量出来的是占位符的宽度，\
         与真实渲染宽没有关系。请把它们改归 `宽度判据::参数化`，并确认另有一条对\
         **拼好的整行**的实测断言：\n{}",
        漏网.join("\n")
    );
    // 「断言恒绿因为循环一次都没进」那条假绿形状的钉子。
    assert!(
        查过的带变量的键 >= 2,
        "两份 .ftl 里一条带变量的文案都没扫到（实际 {查过的带变量的键} 条）——\
         多半是 `全部文案` 的行解析判据坏了"
    );
}
...
fn 每一个键在它所属面板里都不超行数预算() {
    ...
    // 会让下面那条断言永远绿（ADR 0022 的假绿形状，上一批在「语言」行
    // 上抓到过一次）。
    assert!(
        真的量过的条数 > 200,
