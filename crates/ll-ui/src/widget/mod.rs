...
//! 控件库（焦点导航、按钮、输入处理仍然排在 P7 完整批次），但状态栏/
//! 角色面板/背包/装备栏这四块屏幕真正需要的控件必须做扎实，不能是
//! 「把字符串画在固定位置」的临时手法——那类代码会变成后续批次难以
//! 迁移的承重墙（`assets/atlas/` 双份美术、`TurnEngine` 长期滞留在
//! demo 里不并入生产，都是这类「先凑合」代码固化的先例）。
//!
//! 本模块因此提供五个正式控件，各自独立、可单测、可组合：
//!
