...
//! 见 [`BaseQuestIds`] 文档。
//!
//! # 完成条件分档（ADR 0016/0017 三档分级在任务系统上的落点）
//!
//! **编号更正（2026-08-31，批次 25）**：本节标题此前写的是「ADR 0018
//! 三档分级」。三档分级（一档静态声明 / 二档受限公式 / 三档脚本回调）是
//! ADR 0016「mod 性能分档按声明方式，不按作者身份」与 ADR 0017「声明式分档
//! 物化为列式数据」的内容；ADR 0018 讲的是引擎层/玩法层的边界，它只在第三步
//! 判据里**引用**了 0016 的一/二档。分档结论本身一字未改。
//!
...
//! 「已知缺口，记录不硬做」同一条纪律）。
//!
//! > **【2026-08-30 更正：上一段两处已过期。】**
//! > ① **一档已经接线**：`ll_sim::quest::kill_progress_effects` 真的
//! > 在结算击杀计数，`ll_sim::resolve` 对 `Intent::Attack` 与
//! > `Intent::UseSkill` 都会调用它，[`RegisteredQuests`] 就是这条链的
//! > 本 crate 一端。今天判定器**只**认一档，这是运行期的事实，不再是
//! > 「本批次范围之外」。
//! > ② **`ll-script` 这个 crate 已经不存在了**（脚本系统随 ADR 0028
//! > 整体拆除，`ScriptValue` 已改名 `ModStateValue`）。三档今天等的
//! > 不是脚本引擎，见 [`QuestCondition::Script`] 变体文档重写后的理由。
//!
...
pub enum QuestCondition {
    ...
    /// 三档：具名判定函数判定是否完成——处理"拜访某个 NPC 并说出特定
    /// 台词"这类无法穷举成数据的条件。
    ///
    /// 存的是判定函数的标识符（`NamespacedId`），不是函数指针本身——与
    /// `SkillDef.effect` 一样，注册期只存"引用"。注册、查询、无环校验、
    /// 存档往返这半条链路今天全部真实可用；**真正求值它的那一半今天
    /// 不存在**。
    ///
    /// # 它举的那个例子现在有替代品了（2026-08-31 补记）
    ///
    /// 本变体的文档一直用「**拜访某个 NPC 并说出特定台词**」当例子，说它
    /// 是「无法穷举成数据的条件」。**那句话在今天只剩一半成立**：对话系统
    /// 的内容表已经落地（[`crate::dialogue`]，`mods/*/dialogues.json5`），
    /// 「拜访某个 NPC 并说出特定台词」从此是**数据可表达的**——一条选项的
    /// `set-flag` 后果写下一条标志，任务侧读它。
    ///
    /// 推翻这一句的是 `knowledge/design/dialogue-system.md` 五节 2 与
    /// 计划文档 `docs/superpowers/plans/2026-08-31-batch18-dialogue-content.md`
    /// 第七节的挂载点表。**注意后果那一侧（`set-flag`）属于对话的批次 2、
    /// 今天还没有**，因此本变体在那一批落地之前仍然搁置；变的是「将来
    /// 靠什么替代它」这个答案，不是它今天的状态。
    ///
    /// # 这是「刻意搁置」，不是缺陷——理由（2026-08-30 重写）
    ///
    /// **旧理由已作废**：本变体此前的说明与模块文档都写着「真正在运行
    /// 期调用这个回调需要串起 `ll-sim`/`ll-script` 的运行期管线」。
    /// **`ll-script` 这个 crate 已经不存在了**（脚本系统随 ADR 0028
    /// 整体拆除），照着旧理由去判断"它是不是仍然该搁置"会指向一个不
    /// 存在的东西。旧理由原文保留在模块文档「完成条件分档」一节的更正
    /// 框里，追溯用。
    ///
    /// **今天成立的理由**：所有者 2026-08-29 裁定「行为树要数据化，但
    /// 只做参数化不做脚本化」（`knowledge/handoff/2026-08-28-session-handoff.md`
    /// 第〇之二节第 11 条）。那条裁定给出的正是本变体将来该指向的东西
    /// ——**Rust 侧注册的具名判定函数**（一张 `NamespacedId → fn(&World,
    /// EntityId) -> bool` 的注册表），不是一段 mod 提供的源码。内容侧
    /// 早就是这个口径了（`mods/lostland/quests.json5` 文件头），本变体
    /// 携带一个 id 这件事在新口径下**一个字节都不用改**——删掉再写一遍
    /// 是净损失。
    ///
    /// **最小形状**：一张与 [`crate::registry::Registry`] 同期加载的
    /// 具名判定函数表；[`ll_sim::quest::QuestCatalog`] 增加一个
    /// `script_quests()` 之类的查询（与既有的 `kill_count_quests` 并列，
    /// 不改它）；判定点与 `kill_progress_effects` 同一层。
    ///
    /// **落地条件**：与行为树参数化同一批——那一批必须先立起「具名函数
    /// 注册表」这个机件，本变体是它的第二个消费者，届时接上去是加一个
    /// 查询，不是发明一套新东西。在那之前**本体内容不得使用本变体**：
    /// 一条求值不出来的条件挂在任务图上，会让它的后继节点永远解不开
    /// （2026-08-29 审计缺陷第 1 项就是这么发生的，`lostland:finale`
    /// 曾经因此是一个死节点）。这条约束由
    /// `crates/ll-mod/tests/base_quest_graph_completable.rs` 守住。
    /// **mod 不受这条约束**——mod 作者有权声明一条今天求值不出来的条件，
    /// 那是他自己的账。
    ///
    /// # 〔2026-08-31，批次 29〕它举的那个例子已经有替代品了
    ///
    /// 本变体的例子原文是「处理『**拜访某个 NPC 并说出特定台词**』这类
    /// 无法穷举成数据的条件」——**那正是对话**。对话系统的批次 4 落地了
    /// `complete-quest` 后果（`ll_sim::dialogue::DialogueOutcome::CompleteQuest`），
    /// 一条选项直接调既有的 [`ll_sim::quest::mark_quest_completed`]，
    /// 于是这一整类条件从「要一个求值器」变成了「一条数据边」。
    ///
    /// **这不等于本变体可以删**：它服务的是「无法穷举成数据」的**其余**
    /// 那些条件（走到某处、活过某个季节、身上带着某组东西……），那些仍然
    /// 等具名判定函数注册表。变的只是它举的那个例子不再是它独有的理由。
    ///
    /// **也不等于本体现在可以用它**：上一段那条约束原样成立。真要写一条
    /// 「靠对话完成」的任务，今天的形状是**任务照常声明一条求得出来的
    /// 条件、对话另外给一条完成路径**；若将来出现「只能靠对话完成」的
    /// 任务，`base_quest_graph_completable.rs` 那道门要先学会把对话这条
    /// 完成路径并进它的判据——那是一次独立的门禁扩面，批次 29 没有做
    /// （见其计划文档 4.3）。
    Script(NamespacedId),
}
...
/// 节点是树的定义性质，`finale` 有两个父节点，图因此不是树）。
///
/// **【2026-08-30 更正】** 上一段后面原本还有一句「同时演示两档完成
/// 条件：`main_quest_1`/`branch_a`/`finale` 用一档（`KillCount`），
/// `branch_b` 用三档（`Script`）」。**四条现在全部是一档。** 原因是
/// 三档今天没有求值器，而 `branch_b` 是 `finale` 的前置——本体任务链的
/// 终点因此曾经是一个死节点（2026-08-29 审计缺陷第 1 项，所有者裁定
/// 「改条件」）。完整来龙去脉写在 `mods/lostland/quests.json5` 文件头，
/// 三档变体本身为什么保留写在 [`QuestCondition::Script`] 文档。
...
pub struct BaseQuestIds {
    ...
    /// `main_quest_1` 的分支之一：一档完成条件，`target_kind` 与
    /// [`Self::branch_a`] **不同**——同一个 `target_kind` 会让两条分支
    /// 共用同一个击杀计数键、同时达标，理由见
    /// `mods/lostland/quests.json5` 文件头。
