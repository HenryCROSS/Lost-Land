...
pub struct RawRuleModifier {
    ...
    /// 生疏」这类负面天赋），因此**不钳到零**；产出保底在结算侧
    /// （[`ll_sim::rule_modifier::MINIMUM_CRAFT_PRODUCT_COUNT`]）。
    ///
    /// 合并批次订正：本字段此前援引的「`damage_reduction` 允许负值表示
    /// 脆弱」那条先例**已经不在**——脆弱改由 `kind: "vulnerability"`
    /// 表达，`damage_reduction` 随之收窄成非负。这里仍然允许负值，理由
    /// 换成了它自己的那一条：制作产出没有「反方向的变体」可拆，少产出
    /// 与多产出是同一把尺子的两端，不是两件事。
    #[serde(default)]
...
impl RawRuleModifier {
    ...
    fn resolve(&self, registry: &Registry) -> Result<RuleModifier, String> {
        ...
        match self.kind.as_str() {
            ...
            // 不是第二样要学的东西。
            "craft-yield" => {
                self.only(&["recipe_category", "bonus_product_count"])?;
            ...
            other => Err(format!(
                "未知的规则修正 kind {other:?}（只认 resistance/vulnerability/\
                 sneak-attack/inspection-suspicion/inspection-concealment/\
                 advantage/disadvantage/reroll-once/craft-yield）"
            )),
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn 其余八个kind都不接受制作产出加成的两个字段() {
        // 上一条的对偶：新增字段必须被**其余每一个**分支拒掉，否则
        // 「填了但没生效」会静默通过。
        //
        // # 合并批次把这条从四个 kind 扩到八个
        //
        // 本条原来只遍历制作产出加成批次当时存在的那四个 kind。判定系统
        // 落地批次同时（一）新增了 `vulnerability`/`advantage`/
        // `disadvantage`/`reroll-once` 四个 kind，（二）把逐分支手写
        // `reject(...)` 换成了 `RawRuleModifier::only` 白名单——而 `only`
        // 的字段表是一个**硬编码定长数组**，合并时若没把
        // `recipe_category`/`bonus_product_count` 补进去，那两个字段会被
        // 每一个分支静默放行，正是本条要防的失败。遍历全部其余 kind，
        // 让那份字段表漏一条就变红。
        // Arrange
        let mut registry = Registry::new();
        registry.intern(parse_id("m:acid", "伤害类别").unwrap());
