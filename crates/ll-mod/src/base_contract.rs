...
//!
//! # 本模块只是第一层：内容自身的形状归 [`crate::content_audit`] 管
//!
//! 契约解析看的是**句柄结构体点名的那几条 id**，看不见内容自身的
//! 形状——一条内容把某个跨表 `ContentIndex` 引用指向不存在的目标、
//! 或者某个字段全仓库没有任何一条内容给它赋过非默认值，本模块全都
//! 照样放行。那两件事由 [`crate::content_audit`] 在同一条装载管线的
//! 更后一步校验（两者在 `ll_game::content::load_content` 里前后相邻），
//! 见其模块文档「与 [`crate::base_contract`] 的分层关系」一节。
//!
