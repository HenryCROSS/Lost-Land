...
use culture::write_culture_fields;
...
/// 字面值就是唯一权威。
///
/// **2026-08-31 原地更正**：上面划掉的那条单测**从来没有存在过**。
/// 证据：`git log -S 建材不同的两条文化摘要不同 -- crates/` 只命中两个
/// 提交，`4aec07e`（本节所述的那次改动）加进来的是**这句注释本身**、
/// 没有任何 `fn` 定义。也就是说版本 27 这一节从写下的那一刻起，声称的
/// 守门人就是空的——**恰好是它自己下一行警告的那件事**。
/// 现在真正守这一段的是本模块 `tests` 里的单测
/// `建筑类型不同的两条文化摘要不同`（据点建筑批次补上，走
/// [`write_culture_fields`] 的完整字段流）。**这一次这个名字是真的**：
/// `grep -n "fn 建筑类型不同的两条文化摘要不同" crates/ll-mod/src/content_hash.rs`
/// 命中一条 `fn` 定义。
///
/// ---
///
...
mod tests {
    ...
    /// |---|---|---|
    /// | `JoinSettlement` 的 `write_u64(1)` 改成 `write_u64(0)`（与 `SetFlag` 撞判别值） | **不红** | 不红 |
    ...
    fn 后果种类不同的两个对话节点摘要不同() {
        ...
        // Assert（直接对 `write_dialogue_outcome`）：`JoinSettlement`
        // 这一支**必须往哈希里写点什么**。见本测试文档那张表第二行——
        // 这是它唯一真的咬得住的那一半。
        ...
        assert_ne!(
            空手, 写过,
            "JoinSettlement 这一支整支空过 = 它在内容哈希里不存在"
        );
    }
