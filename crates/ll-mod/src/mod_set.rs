...
//!
//! # 生产代码里的唯一调用点（生成期 mod 集合修正批次钉死）
//!
//! 这句「一次性调用」曾经只是注释，而 `ll_game::save::save_game` **每
//! 次存档都调它一次**——拿「玩家现在开着哪些 mod」覆盖掉「这个世界当初
//! 是用哪些 mod 生成的」，玩家中途装的 mod 会永久混进生成期名单且追不
//! 回（`knowledge/audit/2026-08-26-phase-reckoning-p6-p8.md` 三节第 9
//! 项）。
//!
//! 现在 [`GenerationModSet::capture`] 在生产代码里**只有一个调用点**：
//! `ll_game::world::build_new_world` 里绑定 `ll_content::world_identity::WorldIdentity`
//! 的那一行。存档路径不再调用它，读档路径把存档头那一份原样接回来。
//! 这条纪律也不再只靠注释：存档头的四个世界身份字段是 `pub(crate)` 的，
//! `ll-game` 侧唯一的写出入口只接受一份已经绑定好的世界身份，见
//! `ll_content::world_identity` 模块文档「单一真相源」一节。
//!
