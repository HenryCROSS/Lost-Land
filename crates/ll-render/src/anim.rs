...
///
/// 六帧的播放顺序：接触 → 过渡 → 过腿 → 接触 → 过渡 → 过腿 → 循环回
/// 接触。此前只有 `hero_walk_0`/`hero_walk_1` 两帧接触姿态直接互跳，
/// 两帧之间的像素差异（32/384）已经接近「行走对待机」的差异
/// （48/384），观感生硬。`hero_walk_2`/`hero_walk_4` 是脚部朝中线过渡
/// 但仍贴地的姿态，`hero_walk_3`/`hero_walk_5` 是脚部摆到中线附近、
/// 抬离地面 1 像素的「过腿」姿态（`ll-artgen` 的
/// `sprite::decorate_hero_walk` 用 `passing` 参数区分）——六帧沿这条
/// 顺序播放，相邻帧像素差异全部落在 16~26 之间，见
/// `tools/ll-artgen/src/main.rs` 的
