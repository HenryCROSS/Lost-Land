...
fn 三条气候带的树种分布互不相同() {
    ...
    for i in 0..3 {
        for j in (i + 1)..3 {
            assert_ne!(
                r[i], r[j],
                "第 {i} 条与第 {j} 条气候带的树种比例完全相同（{:?}）——气候多半没有真的接上",
                r[i]
            );
        }
    }
...
fn 相邻格子长树与否的相关性接近相互独立() {
    ...
    // 的那个维度会明显倾向「相邻格同结果」。批次 28 实测到的后果是画面上
    // 的纵向条纹（32% → 47%）。
    //
    // `position_digest` 因此把长字符串域名放在最后。把 `x`/`y` 挪到最后
    // 一位，本条当场红（实测数字见计划文档十节）。
    //
    // 理想值：两格独立时「都有树」的概率是 p²，「结果相同」的概率是
    // p² + (1-p)²。p = 0.62 ⇒ 0.3844 + 0.1444 = 52.9%。门槛取 +8 个百分点
    // （与批次 28 那条同一个宽容度）。
    let (forest, _) = forest_and_grass();
    let size = size();
    let has = |x: i32, y: i32| {
    ...
    for (i, name) in 方向.iter().enumerate() {
        let permille = same[i] * 1000 / total;
        println!("{name}相邻同结果 {permille}‰（相互独立时 {理想}‰）");
        assert!(
            permille < 门槛,
            "{name}相邻格有 {permille}‰ 取到同一结果（相互独立时应为 {理想}‰，门槛 {门槛}‰）\
             ——哈希混入次序多半被人改了，见 `position_digest` 文档"
        );
    }
}
