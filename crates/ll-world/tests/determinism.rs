...
//
// # 世界生成参数落地批次：这一次也**重冻了**，理由与 NPC 生成批次同型
//
// `WorldState` 新增顶层字段 `terrain_shape`（地形形态参数：海平面 /
// 山地阈值 / 倍频层数 / 大陆尺度缩减档位），`WorldState::hash` 通过
// `write_terrain_shape` 把它的四个字段全部混入。混入代码在
// `for agent in self.actors.iter()` 循环体**之外**，每次 `hash()` 都会
// 执行——哪怕四个值全取默认，也会往哈希器里多写四个整数，摘要必然改变。
//
// 四步核验（真实执行，不是推断）：
//
// 1. **确认基线红**：改动落地后跑 `cargo test -p ll-world --test
//    determinism`，本条测试红，实测摘要 `17_228_492_522_544_021_674`，
//    期望值仍是旧常量 `13_932_142_645_965_877_185`。
// 2. **把改动关掉，确认精确回到旧值**：只把 `WorldState::hash` 里那一行
//    `write_terrain_shape(&mut hasher, self.terrain_shape);` 注释掉，
//    其余全部改动（`GenParams` 拆出 `TerrainShape`、
//    `TileableNoise::shrink_continents`、新增的 `terrain_shape` 字段与
//    它的序列化、四档形态预设）原样保留 —— 本条测试当场转绿，
//    摘要与旧常量 `13_932_142_645_965_877_185` **逐位相同**；同一次
//    验证里 `crates/ll-sim/tests/replay.rs` 的
//    `EXPECTED_REPLAY_DIGEST` 也同时精确回到它的旧值
//    `17_219_713_135_340_447_375`。这证明本批次那一堆结构性改动在默认
//    参数下**没有改动任何一格地形**，摘要的变化只由新增的这一处哈希
//    输入引起。
// 3. **恢复那一行**，摘要稳定落回步骤 1 实测的新值。
// 4. **两个独立进程复现**：把这条测试单独跑了两次独立的 `cargo test`
...
//
// # 气候条带批次（规格 §7.1，2026-08-27）：**重冻了**，四步证据如下
//
// 变化有两个独立来源，两者都必须重冻，不是「测试挂了就抄实际值」：
//
// 1. **地形判定的输入变了**：`height_to_terrain` 的第四段（海岸带以上
//    第一段陆地）现在按气候带三选一——干热带出沙漠、极地带出冻原、温带
//    仍是草地（见 `ll_world::climate` 与 `ll_world::generate`）。
// 2. **`write_terrain_shape` 多混入一个字段**：
//    `TerrainShape::climate_band_width`。它必须混入，理由见该函数文档。
//
// 四步重冻（每一步都真实执行过，不是推断）：
//
// ① **基线红**：改动落地后本条报
//    `left: 7252656868039639873, right: 17228492522544021674`。
// ② **关掉后精确回到旧值**：把 `TerrainShape::DEFAULT_CLIMATE_BAND_WIDTH`
//    临时改成 `0`（该取值是**精确恒等**：`band_from_warmth` 用严格不等号，
//    带宽为零时两条判据恒假、整图温带），并且此时 `write_terrain_shape`
//    尚未混入新字段——本条**当场转绿**，摘要与旧常量
//    `17_228_492_522_544_021_674` 逐位相同。这证明本批次横跨十余个文件的
//    结构性改动（新增 `climate` 模块、`TerrainShape` 加字段、
//    `BaseTerrainIds` 加两种地形、`terrain_at_coord` 多一个入参）在气候
//    关闭时**没有改变任何行为**。
//    同一步还单独确认了一件事：新增的 `lostland:desert`/`lostland:tundra`
//    两条注册**追加在既有 17 种之后**，前 17 种的 `ContentIndex` 一位没动
//    ——起初误插在 `snow` 之后，把其后九种地形的索引整体平移了 2，
//    `crates/ll-sim/tests/replay.rs` 当场抓到；挪到末尾后那条也精确回到
//    了它的旧值。
// ③ **恢复**：带宽改回 250，并把 `climate_band_width` 混入
//    `write_terrain_shape`。
// ④ **两个独立进程复现**：新值 `10_180_278_885_427_934_050` 在两次彼此
...
//   而且这次用**两条反例**证的，不是「跑一遍没红就当没事」
//
// 本批次动了两样按理会打飞世界摘要的东西：
//
//   ① 据点的建筑布局（间距从「恒 1 格」改成「按人口分档 + 每三栋插一
//      条街」，`ll_world::settlement::building_origin`）——地形层的东西，
//      而本条的 `hash()` **确实**把每个常驻区块的地形逐格混进去。
//   ② 世界里第一次出现了世界生成期就摆下的家具（`ground_items` 里带
//      `placed: true` 的 `GroundItemStack`）。
//
// 改完之后本条**当场就是绿的**。交接文档点名过这种情形的第二类陷阱：
// **「基线没红」可能只是因为那个测试的世界里根本不存在这类对象**。因此
// 两条都用反例证伪，逐条实测：
//
// - **反例 A（据点）**：在 `ll_world::settlement::stamp_settlement` 函数体
//   开头临时插一行 `grid.set_terrain(grid.world().wrap(0, 0),
//   ctx.ids.wall_stone);`——也就是让**每一次**据点铺设都往窗口左上角写一
//   堵石墙。本文件十一条**仍然全绿**，摘要逐位不变。这证明的比「新布局
//   没影响」更强：**本条测试的世界里一次 `stamp_settlement` 都没发生过**
//   （它走 `WorldState::new`，从不建档、不跑编年史，`SurfaceStore` 的
//   `chronicle` 恒为 `None`）。随后删掉那一行恢复原状。
// - **反例 B（家具）**：在 `crate::state` 的 `write_item_stack` 函数体开头
//   临时插一行 `hasher.write_u64(0xDEAD_BEEF)`。本文件十一条仍然全绿——
//   与归属批次当初记录的结论一致：**这个世界里一个 `ItemStack` 都没有**。
//   这一次是重新实测的，不是引用上一批的记录。
//
...
// `mods/lostland/cultures.json5` 追加第七份文化并改了四处
// `founder_races`。三条黄金基准里**只有
// `crates/ll-game/tests/populated_determinism.rs` 那条红了**，本条与
// `replay.rs` 那条都是绿的。
//
// 「绿」在本仓库不算证据——交接文档点名过第三类陷阱：**绿可以被「那段
// 代码根本没执行」解释掉**（本会话真的踩过一次：注入点落在
// `#[cfg(test)]` 模块里）。因此这一次先立**对照组**，再谈结论：
//
// - **对照组（已知会红的那一个）**：把 `TerrainShape::default()` 的
//   `sea_level` 从 400 临时改成 401——那是一处**生产代码**（
//   `crates/ll-world/src/terrain_shape.rs`），不在任何 `cfg(test)` 里。
//   本条当场红：`left: 5761068368444813888`、
//   `right: 11270479921196970914`。同一处改动也让
//   `replay.rs` 那条红（`left: 8781493854886781786`、
//   `right: 11222878776777704235`）。**两条测试都真的在执行，也真的
//   对世界的改变敏感。** 随后把 401 改回 400，两条都恢复绿。
// - **结论**：它们的绿是「够不到」，不是「没执行」。够不到的理由是
//   结构性的——本文件与 `replay.rs` **一次都没有装载过 `mods/`**：
//   两个文件里 `load_content`/`LoadSession`/`Chronicle`/`CultureTable`
//   的代码命中数为零（`determinism.rs` 只有一处出现在注释里）。它们的
//   世界由测试自己手搓，种族与职业索引是测试自己造的固定值，
//   `mods/lostland/*.json5` 里写什么都到不了它们。
//
// 这与上面六条「本条够不到」的记录是同一个形状，只是这一次那个空洞
// **已经有落点了**（见下方补记）：本批真正被重冻的就是那条。
//
...
// 3. **恢复**那一行。
// 4. **两个独立进程复现**新值。
const EXPECTED_WORLD_DIGEST: u64 = 9_641_812_801_195_310_050;

// # 【2026-08-31 补记】上面这六条「本条够不到」的记录，现在有落点了
//
// 从「等级与经验」到「据点建筑类型」，本常量的文档注释里累积了**六条**
// 「本批次没有重冻／只重冻了一个空表的长度标记，同一条理由」的记录。
// **每一条单独看都是诚实的核实结论**——都做了反例验证，都没有把「跑一遍
// 没红」当成「覆盖到了」。但六条叠在一起，它们是**同一个空洞被六个互不
// 相干的批次反复撞到**：本条的世界由 `WorldState::new(..)` 直接构造之后
// 一行都不再动，零 `actors.spawn`、零 `stamp_settlement`、零 `ItemStack`、
// 零势力。
//
// 批次 22 新增了**第三条**黄金基准来接住这一块：
// `crates/ll-game/tests/populated_determinism.rs` 的
// `EXPECTED_POPULATED_WORLD_DIGEST`——它的世界全部走生产路径构造
// （`build_new_world` + `materialize_nearby_settlements`），实测含
// 已物化据点 2 座、实体 29 个、地面物品 126 件（全部带 `Owner::Faction`
// 且 `placed`）、势力 255 个。上面六条各自够不到的东西，在那一条上
// **逐类做过反例验证、逐类都红**（表在那个文件的模块文档里）。
//
// **本条一个字都没改，也不该改**：它守的是「地形怎么生成」那一段，便宜
// （不装载 mods、不跑编年史）且仍然有效，日常改动应该先撞到它。新的那条
// 是最外面那一层网，不是它的替代品。
//
// 空洞的实测量化见
...
fn 新增势力后世界哈希必须变化() {
    ...
    //
    // 反例验证（ADR 0022）：把 `FactionTable::write_hash` 里除长度以外
