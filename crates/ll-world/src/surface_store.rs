...
/// `stream_neighborhood` 之后，本类型的 panic 前提在**渲染路径**上不会
/// 被触发——它仍然保留（而不是改成静默兜底），是为了在这条纪律
/// 被违反时（例如调用方漏调了 `stream_neighborhood`，或半径给小了）
/// 尽早、吵闹地暴露出来，而不是把一格看不见的黑块悄悄留在画面上。
///
/// # 但「调用方自己保证前提成立」不适用于非渲染路径（2026-08-27 更正）
///
/// 上一节此前写的是「正常游玩路径下不会被触发」。**这句话被实机证伪
/// 了**：常驻集合只围着玩家维护，而 AI 结算对常驻情况零过滤，一个远处
/// 据点的卫兵在自己脚下的区块被 LRU 驱逐之后照样会跑到 `compute_fov`
/// 里来，于是本类型当场把游戏崩掉（所有者实测：
/// `TorusPos { x: 1008, y: 0 } 所属区块尚未加载`）。
///
/// 对一个离屏 NPC 而言，「脚下的地形不在内存里」不是纪律被违反，是**正常
/// 状态**——没有任何调用方该为它去 `stream_neighborhood`。因此这类调用方
/// 不能靠本类型的 panic 来守，必须**在构造本类型之前**先自查一次：
/// [`crate::sight_residency::fov_neighborhood_resident`] 是那份判据的唯一
/// 实现，`ll_sim::ai_query::nearest_visible_actor` 与 `ll_sim::apply`
/// 落地 `Effect::MarkExplored` 两处共用它。
///
/// 本类型的 panic 本身**一个字都没改**：它继续守渲染路径那条纪律，那正是
/// 它该守的东西。
