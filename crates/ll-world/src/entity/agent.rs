...
pub struct Agent {
    ...
    ///
    /// 暴击率（每点 +5‰）已接线——见 `ll_sim::combat::crit_chance_permille`/
    /// `ll_sim::resolve::resolve_attack`「暴击」一节（幸运接线批次）。
    /// 两者定义在下游的 `ll-sim`（`ll-world` 不能反过来依赖它，见依赖
    /// 方向 `ll-world` ← `ll-sim`），这里只能用反引号纯文本指向，不能
    /// 用 `[`...`]` intra-doc link——那会在 `cargo doc` 时报「no item
    /// named `ll_sim` in scope」。
    /// 仍未落地的（均详见 `attribute-system.md` 「五、幸运」）：优势
    /// 掷骰（每满 20 点多掷一次取较优）、掉落品质权重、稀有事件触发
    /// 权重——本批次刻意只接一个消费者（暴击率，「战斗结算里现成的
    /// 判定点」），其余三项各自需要一套目前还不存在的机制（判定系统
    /// 本身/掉落表/随机事件表），留给各自的系统落地批次。
    ///
    /// # 裁定：不并入 [`super::AttributeKind`]（幸运接线批次）
    ///
    /// 幸运本可以像力量/敏捷/体质/智力/意志/魅力那样并进
    /// `AttributeKind`，换来的好处是幸运戒指/祝福术这类装备/技能加成
    /// 能复用 `Agent::active_stat_modifiers`（按 `AttributeKind` 索引）
    /// 这条现成通道；代价是 `AttributeKind` 被 `derive_stats`/
    /// `StatBonus`/内容值哈希/`ll_content::remap` 多处穷尽匹配
    /// （`match`/`for kind in [...]` 一类写法），新增一个变体意味着这些
    /// 地方全部要跟着改一遍，而这批任务书只要求接一个消费者（暴击率），
    /// 不要求幸运能被 buff。
    ///
    /// 本批次选择**不并入**，保持 `luck` 是 `Agent` 上的独立字段——
    /// 「幸运是天生的、改不了的」本身也是一种合理设计（YAGNI：当前
    /// 没有任何已裁定的设计要求幸运能被装备/技能加成，见
    /// `attribute-system.md`「五、幸运」全文没有一处提到幸运的来源是
    /// 「六维属性 + 装备/状态效果」，反而通篇只讨论「幸运值本身如何
    /// 改变判定形状」，暗示它更接近种族/背景决定的固定值,而不是六维
    /// 属性那种可堆叠养成线）。真要支持「幸运戒指」一类装备加成时，
    /// 再权衡「幸运并入 `AttributeKind`」与「给 `Agent` 单开一条独立的
    /// 幸运修正通道（类似 `active_stat_modifiers` 但只服务这一个字段，
    /// 不放大 `AttributeKind` 的穷尽匹配面）」两条路径，不在本批次提前
    /// 决定。
    pub luck: i32,
