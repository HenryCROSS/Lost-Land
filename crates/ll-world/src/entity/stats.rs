...
pub struct BaseStats {
    ...
    /// 魅力：招募随从、交易议价、随从士气。
    pub charisma: i32,
    /// 幸运：暴击判定的修正（每点 +1 点，`ll_sim::combat::crit_attacker_modifier`）
...
impl BaseStats {
    ...
    /// 具体数值却假装它有出处。
    ///
    /// **幸运取零，不是十**：幸运走原始值语义（见 [`Self::luck`]
    /// 文档），没有六项主属性那套调整值公式的「基准点」概念可言——
    /// `ll_sim::combat::CRIT_BASE_CHECK_MODIFIER`（定义在下游的
    /// `ll-sim`，`ll-world` 不能反过来依赖它，见依赖方向 `ll-world` ←
    /// `ll-sim`，这里只能用反引号纯文本指向，不能用 intra-doc link）
    /// 文档「幸运怎么进式子」一节论证的正是这一点：幸运的原始值**本身
    /// 就是一个相对基准的增量**，与骰子修正同一种量，因此基准取零、
    /// 每点换一点修正，不再过一道 `(属性 − 10) / 2` 的调整值换算。
    /// 判定系统迁移批次之前，这里的零基准还额外承担着「零幸运 ⟺ 零
    /// 暴击率，既有确定性测试不受影响」那条理由；暴击换成对抗判定之后
    /// 零幸运对应 `4.84%` 而不是零，那条理由随之作废，留下的只有上面
    /// 这条量纲理由——它才是本来就该独立成立的那一条。
