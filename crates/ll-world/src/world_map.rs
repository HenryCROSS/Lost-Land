...
//! # 三条设计裁定
//!
//! 1. **档位是离散的**（[`ZOOM_LADDER`]）。连续缩放意味着「一格覆盖多少
...
/// 因此视野恒为 32×24 格——与本批之前 `WORLD_MAP_DOWNSAMPLE = 2` 时的
/// 格数完全一致，面板布局不因本批而变。
///
/// 取 2 的幂且逐档减半：每一档「一格覆盖的瓦片数」都能被上一档整除，
/// 因此放大再缩小回来必然回到**逐位相同**的画面，不会因为取整方式攒下
/// 漂移。默认布局下四档分别是一格 96 / 48 / 24 / 12 个瓦片——第 1 档
/// 恰好是「一格一个区块」，正是「在地图上挑一个区块出生」需要的粒度。
pub const ZOOM_LADDER: [u32; 4] = [8, 4, 2, 1];
...
impl WorldMapSlice {
    ...
    /// # 为什么取格子**中心**而不是左上角
    ///
    /// 一格在最远档位下覆盖 8×8 个采样点 = 两个区块见方，左上角与中心
    /// 分属不同区块。玩家点一格时心里指的是「这一片」，取中心比取角落
    /// 更接近那个意思；取左上角还会让整张图系统性地偏向左上半格。
    ///
...
mod tests {
    ...
    fn 从原点往西平移一格视野中心绕到世界东侧() {
        ...
        // ——那条测试因此咬不住钳制。**这不是设想：本批做 ADR 0018 反例
