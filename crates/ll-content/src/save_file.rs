...
/// # 7 -> 8（NPC 姓名批次，2026-09-02，批次 34）——**第二次**「布局没变，
/// 门禁的过度近似让它必须升」
///
/// 与 4 -> 5 那一次**同形**（见上面那一段），连类型都撞上同一个：
/// [`ll_world::culture::CultureAttrs`] 多了一个 `naming` 字段，
/// `CultureTable` 的字段序 10 -> 11，`CultureNaming`/`PhonemeTables`
/// 两个新类型进入门禁算出来的闭包。
///
/// **存档主体的字节布局没有变。** `CultureTable` 跟着编年史走，而
/// `SurfaceStore` 的手写 `SurfaceStoreData` repr 里根本没有 `chronicle`
/// 字段（ADR 0009：编年史是种子的纯函数，读档后由
/// `ll_game::rebuild_chronicle` 重跑）。门禁的闭包是从**声明的字段**
/// 机械求出的，看得见 `SurfaceStore.chronicle` 这个字段、看不见手写 repr
/// 把它丢掉了——这是**同一处过度近似的第二次命中**，如实登记，
/// 不是缺陷发现。前一次记在上面 4 -> 5 那一段，两处互相指向。
///
/// **与 4 -> 5 不同的是：本批没有第二个「真事」理由。** 那一次派生的据点
/// 布局真的变了（街道插入），本批**什么派生形状都没变**——证据是
/// `EXPECTED_POPULATED_WORLD_DIGEST` 在本批的 NPC 姓名那一半下**逐位
/// 不变**（姓名是渲染期现算，不进世界状态）。因此这一版的诚实说法是：
/// **升版本是为了过那道阻断门禁，而不是因为老存档会被误解析。**
///
/// **那么老存档实际会怎样？** 被拒——但**拒它的不是这一行**。
/// 本批把 `ll_mod::content_hash::CONTENT_HASH_ALGORITHM_VERSION` 从 34
/// 升到了 35（既有表加字段，ADR 0027 强制），而
/// [`crate::load_error::check_content_hash_algorithm`] 对**版本不等**
/// 一律 `Rejected`，且它排在读档流程里更靠前的位置。也就是说：任何一次
/// 内容哈希算法升级都会让老存档打不开，这一行的 7 -> 8 **没有增加任何
/// 额外损失**。这一段是为了让下一个人不必自己重新推一遍。
///
/// **仍然不配迁移函数**，与前七次一样（所有者裁定「老存档去掉就好了」）。
