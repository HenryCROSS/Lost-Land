...
//! 算敌对。本文件两条用例分别钉住势力那一半判据的两端。
//!
//! **本文件验的是势力那一半，文化那一半在 `culture_hostility.rs`。**
//! 文化归属与敌对判定批次之后，「已声明」不再只有势力一种来源：一份
//! 文化可以在 `cultures.json5` 里声明对另一份文化、乃至对「无文化」的
//! 敌意，达到阈值同样算敌对。本文件第一条用例里那两个实体的
//! `affiliations` 是空的，因此文化判据取的是「无文化 vs 无文化」——
//! 敌意 0，不敌对，与本文件落地时的行为逐位相同。玩家那一侧的空
//! `affiliations` 也**不是夹具偷懒**：所有者裁定「玩家可以没有势力
//! 归属」，`build_player_agent` 至今写死 `Vec::new()`，见
