...
//
// 〔2026-09-02，批次 34（NPC 姓名 + 玩家初始钱包）〕**又变了一次**，
// 起因是**玩家初始钱包**：所有者裁定开局给 1000
// （`ll_game::world::PLAYER_STARTING_WALLET`），此前是 0。
// `Agent::wallet` 进 `WorldState::hash`，而本条基准的世界里有一个玩家，
// 因此它必然变。
//
// **同一批的另一半（NPC 姓名）一个字都没动本条**，那是刻意的、也是那
// 一半的验收判据：姓名是**渲染期现算**（`ll_world::naming::agent_given_name`），
// 不进世界状态、不进存档，`Agent` 一个字段都没加。它若动了本条，说明
// 有渲染期的东西漏进了世界状态——那是缺陷，不是该重冻的理由。
// 「没红」的证伪由本次第 ① 步给出：**同一次运行里**，钱包那一行改了本条
// 就红，可见本条对「`Agent` 的字段值变了」并不免疫。
//
// **四步同样跑满**：
//
// 1. **基线红**：`left: 5461649738848521059` /
//    `right: 10943522416722902806`。七条存在性断言全部通过，panic 落在
//    最后那行 `assert_eq!(world.hash(), ..)`——世界没有变空。
// 2. **把改动关掉，精确回到旧值**：只把 `build_player_agent` 那一行
//    `wallet: PLAYER_STARTING_WALLET` 改回 `wallet: 0`（常量本身、
//    NPC 姓名那一整半、内容里的 `naming` 声明全部保留），本条**绿**，
//    摘要精确等于 `10943522416722902806`。这一步证明摘要变化只来自玩家
//    钱包这一个值，不掺杂本批另一半带来的任何漂移。
// 3. **恢复**那一行。
// 4. **两个独立进程复现**新值 `5461649738848521059`。
const EXPECTED_POPULATED_WORLD_DIGEST: u64 = 5_461_649_738_848_521_059;
