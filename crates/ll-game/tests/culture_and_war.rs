...
/// 本文件用的世界种子。三个独立种子（8 / 29 / 32）实测都满足下面这
/// 几条断言（五条测试各跑一遍全绿）；固定成一个只是为了失败时能原样
/// 重跑。
///
/// # 为什么换过一次种子（2026-09-01，跨表撞名门禁批次）
///
/// 原值是 `20260826`，另两个记在案的是 `7` / `99`。跨表撞名普查发现
/// 本体地形「雪地」与本体天气「下雪」一直共用 `lostland:snow`，天气
/// 一侧改名成 `lostland:snowfall` 之后**多出了一个 id**，其后全部
/// `ContentIndex` 整体 +1（`lostland:snowfall` 落在 34，
/// `lostland:dwarf` 由 113 变 114）。这与
/// `populated_determinism.rs` 文件头「新增内容为什么会动这个摘要」
/// 一节记的是同一条既有性质，不是本文件独有的意外。
///
/// 平移本身不改任何权重、不改 `founder_race` 的随机流
/// （`DetRng::for_entity(world_seed, FOUNDER_RACE_STREAM_ID, site)`
/// 与内容索引无关），但它**换掉了这一局生成出来的世界**，于是原来那
/// 三个种子里「恰好被哥布林攻灭的那座矿城是矮人开的」与「三百年里出现
/// 过一次同族占领且那座城活到最后」这两件靠运气的事不再同时成立
/// （`20260826` 两条都不成立，`7` 与 `99` 只剩同族占领那条不成立）。
///
/// **换种子是这条断言自己的注释预先指定的补救方式**（见下面
/// [`矮人矿城被哥布林部落攻灭`] 里那句「换一个种子而不是删掉这条
/// 断言」）：一条都没删、一条都没弱化，五条验收线仍然逐条要求成立。
/// 新的三个候选是扫描 1..=120 号种子、逐个跑真实内容与真实
/// `build_new_world` 挑出来的（共 19 个满足，取前三个）。
const SEED: u64 = 8;
