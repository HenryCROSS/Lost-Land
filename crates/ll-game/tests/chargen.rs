...
fn 种族与职业清单都是从注册表现查出来的() {
    // 正面证据：本体的九个种族与十三个职业都在清单里，且清单里的每一项
    // 在各自的内容表里都查得到定义。**不断言条数**——见模块文档，以及
    // 本函数末尾那段更正（批次 24 抓到本文件自己漏掉了这条纪律一次）。
    // Arrange
    let content = content();
    ...
    // 〔2026-08-31，批次 24 更正〕这里原本是
    //
    //     assert!(roster.professions().len() > roster.races().len(),
    //             "本体职业（13）比种族（4）多，清单若反过来说明查错了表");
    //
    // **那条判据的前提在本批当场变成了假的**：本体种族从 4 加到 9，
    // 而这个测试装的是整个 `mods/`（含 `example_mod` 的 7 个种族与 2 个
    // 职业），于是 16 个种族 > 15 个职业，断言翻转。
    //
    // 它不是被「删掉让它变绿」的：**它要守的那件事（清单查错了表）本来
    // 就已经被上面两个 `is_defined` 循环守住了**——真把两张表接反，
    // `race_table.is_defined(<职业索引>)` 立刻为假。条数大小关系只是那件
    // 事的一个**代理量**，而且是一个依赖内容条数的、迟早会翻转的代理量
    // （正如本文件顶上模块文档自己写的「不能写成『断言有 4 个种族』」
    // ——那条纪律当初漏掉了这一行）。
    //
    // 换成一条不随内容条数漂移、且真的有独立鉴别力的：**两份清单不许有
    // 任何一个索引重合**。种族与职业共用同一个 `ContentIndex` 号段，
    // 把同一张表喂给两边时这一条会当场红，而条数大小关系那一条在那种
    // 情形下反而是绿的（两份一样长的清单，`>` 为假……只有在长度相等时）。
    for race in roster.races() {
        assert!(
            !roster.professions().contains(race),
            "同一个 `ContentIndex` 同时出现在种族清单与职业清单里——\
             两边喂的是同一张表"
        );
    }
}
...
fn 角色创建的三项选择都能换出不同的取值() {
    ...
    // Assert：本体有 9 个种族、13 个职业、2 个性别，三项都拨得动。
    // 这里比的是「> 1」而不是条数本身，正是模块文档那条纪律。
