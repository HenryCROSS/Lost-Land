...
//!
//! # 基准重冻记录（2026-09-01，跨表撞名门禁批次）
//!
//! `surface_preview.png` 重冻过一次，`6208 / 479232` 个像素。**判断过
//! 是有意还是缺陷**（本目录 README 的硬规矩），结论是有意、可解释：
//!
//! - **改了什么**：那栋木屋的门从右上换到了右下（原来右下是一扇窗）。
//!   除此之外整张图逐位相同——玩家出生点、六格林子、树、两个 NPC、
//!   地上四堆物品、锻炉、据点轮廓全部没动。
//! - **为什么**：跨表撞名普查把本体天气「下雪」从 `lostland:snow`
//!   改成了 `lostland:snowfall`（它与本体地形「雪地」撞了名，见
//!   `ll_mod::content_audit::detect_table_define_collisions`）。这**多出
//!   一个 id**，其后全部 `ContentIndex` 整体 +1。
//! - **索引平移为什么会动地面上的东西**：`ll_world::resource::node_roll`
//!   把资源节点的随机流号定义成
//!   `RESOURCE_NODE_STREAM_ID.wrapping_add(资源的 ContentIndex)`
//!   （`resource.rs:517`）——资源索引一平移，**整张资源图重掷**，于是
//!   候选点的资源画像、文化权重、据点推演全部跟着变。实测：编年史站点
//!   数 261 → 262、事件数 325 → 315、那座据点的 `WorldId` 88 → 92。
//!   门的位置由 `WorldId` 派生，因此跟着换了一格。
//! - **不是缺陷的依据**：把那一个字面量改回 `"snow"`（其余改动全部保留）
//!   之后，本文件三条基准**全部逐位变绿**——差异只来自那一次改名，不
//!   掺杂任何渲染路径的行为漂移。
//!
//! # 逐位确定性：实测结论
//!
