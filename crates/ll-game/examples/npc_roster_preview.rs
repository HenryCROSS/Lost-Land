...
//! # 图怎么读
//!
//! 一行是一个种族，一列是一个职业。第一列**不指向任何职业**，是那个
//! 种族的裸身子——把它和同一行右边的格子对照，就能看清职业那一层改了
//! 什么。
//!
//! 有合成图的种族（本体四族）与没有的（示例 mod 的半精灵）在这张图上
//! 看得出是两种画法，这正是回退链在工作的证据：
//!
//! - **本体四行**走的是「种族 × 职业」合成图——衣服/帽子/袖子换成职业
//!   签名色，胸口一枚徽记，手里一件工具，而**肤色、发色、耳型、胡子、
//!   裤子、鞋一个像素都没动**（那是种族那条轴）。
//! - **半精灵那一行**没有合成图，落到最后一段「种族身子 + 职业挂件」的
//!   分层合成：整行身子一模一样，只有胸口那块 6×6 徽记在变。
//!
//! # 它证明什么，不证明什么
//!
...
//! 断言本体每个种族/职业都有自带贴图、每个种族×职业组合都查得到合成图、
//! 52 种组合两两之间至少差四分之一张图。两边合起来才是完整证据。
//!
//! # 名册的内容不在这里写死
//!
...
fn npc_keys(
    ...
    mine.iter()
        // 被压制的那一层整个不画——与生产路径上的 `push_surface_draw`
        // 第一句逐字一致。身子命中「种族 × 职业」合成图时，职业挂件层
        // 必须让位，否则这张预览图会画出屏幕上根本不存在的第二枚记号。
        .filter(|draw| {
            !draw
                .superseded_by
                .iter()
