...
/// 存档名的长度上限（字符数）——**输入框自己放得下**这么多。
///
/// # 这个理由此前写错了（规格 W4）
///
/// 原文是「够认出『哪一份是哪一份』，又不至于把那块居中面板撑破」。
/// **后半句从来不成立**，`knowledge/design/ui-and-navigation.md` §8.3 的
/// O-8 算过：24 个汉字 = 336px，已经占掉当时 500px 内容宽的三分之二，
/// 再加上时间戳与模式就是 +51.4px，**这个上限恰恰没能挡住它**。
///
/// 而今天连那个后果本身都不存在了：批次 19 之后模态屏面板**按内容
/// 伸缩**（`ll_ui::screen` 的 `max(SCREEN_WIDTH, 本屏最长行 + 2×padding)`，
/// 撞到窗口边缘才改走换行），存档列表那一行因此要么被面板容下、要么在
/// 面板内换行，**再长也撑不破**。
///
/// # 为什么不改成按渲染宽度判
///
/// 规格 W4 给了两条路，并建议 (a)「上限改成按渲染宽度判」。本批取的是
/// (b)「保留字符上限、把理由改对」，理由：
///
/// 1. **(a) 要解决的问题已经没了**——见上一段，面板撑不破了。
/// 2. **(a) 会引进一个更大的新问题**：把输入长度上限接到字体度量上，
///    等于让「还能不能再打一个字」取决于玩家选的语言、字号与所用字形
///    （同样 24 个字符，中文比英文宽一倍）。玩家换一次界面语言，已经
///    存在的存档名可能就超限了。
/// 3. **(b) 更容易反转**：真要按宽度判，改的是这一个常量的消费点
///    （`crate::save_name`），本条注释原样删掉即可。
///
/// 上限本身**仍然有意义**，只是意义换了一个：文件名主干要落在文件系统
/// 的路径长度限度里，而且一个几百字的存档名在列表里没有任何可读性。
