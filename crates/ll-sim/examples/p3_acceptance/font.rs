...
/// 音素表覆盖的十七个大写字母、`C`/`Q` 两个额外字母、以及词间分隔用
/// 的空格。
///
/// `C`/`Q` 不在音素表里（该表只用于拼合 NPC 姓名），是专为
/// `crate::push_death_message` 的「GAME OVER」/「ESC TO QUIT」提示新增
/// 的——玩家死亡时需要能拼出「ESC」「QUIT」，音素表原有的十七个字母
/// 凑不出这两个词。多出这两个字符不影响姓名生成（姓名从不会用到它们），
/// 也不算「新造一套字体」，只是给已有的极简点阵字体多刻两个字形。
///
/// 顺序即后续栅格化时在字体行里从左到右摆放的顺序，任意顺序都合法，
/// 这里按数字在前、字母其次、空格收尾，只是方便人工核对时定位。
pub(crate) const CHARSET: &str = "0123456789ABCDEGIKLMNOQRSTUVZ ";
