...
/// 第一次获得某个副职时一次性授予的**属性点**数量。
///
/// # 这个数是占位值
///
/// 项目所有者的裁定只有「副职还是给点数好了」这一句，具体形状是
/// 「你自己先做一份，以后我再改」。因此下面这两个数**没有任何数据
/// 支撑**，是两个纯内容参数，改它们一行逻辑都不用动——量级选取的
/// 依据只有下面这一条，写下来是为了将来改它的人知道当初在权衡什么。
///
/// # 为什么比一级少（升级是 2 属性点 + 1 技能点）
///
/// 见 [`ll_world::entity::Agent::ATTRIBUTE_POINTS_PER_LEVEL`]。副职的
/// **主要**奖励不是点数，是 `SubclassDef::traits`（本体四条制作副职各
/// 带一条 `RuleModifier::CraftYield` 的精通天赋，`5a98d8e`）。点数在
/// 这里是**附赠**，把它定成整整一级会让「获得副职 ≈ 升一级 + 白送一
/// 条天赋」，天赋那半反而被点数盖过去。技能点取满 1（与一级相同）是
/// 刻意的：新副职带来一批新天赋/新技能，当场能买一个是这份奖励读起来
/// 最顺的那一半；属性点取半级（1 而不是 2）负责把总量压回「不到一级」。
///
/// # 上限意义上的总量
///
/// 一个角色一辈子最多从这条路拿 [`MAX_SUBCLASSES`] 份
/// （见 [`ll_world::entity::Agent::subclasses_ever_granted`]：放弃再
/// 赚回**不会**再发一次），当前即 3 × (1 属性点 + 1 技能点)。
///
/// # 为什么是引擎常量，不是 `SubclassDef` 上的一个内容字段
///
/// 与 [`ll_world::entity::Agent::ATTRIBUTE_POINTS_PER_LEVEL`] 逐字
/// 同一条判据（那个常量的文档已经把这场论证打完了一次）：项目所有者
/// 的裁定只说「给点数」，没有要求**每个副职给的数量各不相同**；本体
/// 六条副职也确实没有一条需要与别的不同——它们之间的差异载体是
/// `SubclassDef::traits`（已经是内容可声明的），不是点数。给一个当前
/// 没有任何内容作者要求过的量提前开注册位，正是 ADR 0021 与 YAGNI
/// 同时点名的那种投机式抽象。
///
/// 这条选择还有一半**不对称的代价**必须算进来：内容字段一旦落地，它
/// 就要混进内容值哈希、要递增 `CONTENT_HASH_ALGORITHM_VERSION`、要进
/// `content_schema` 与字段覆盖门禁——而**撤掉**它比加上它贵得多（存量
/// 存档的内容摘要已经把它算进去了）。反过来，将来真有 mod 要按副职
/// 区分时，在 `SubclassDef` 上加一个 `Option<…>`、`None` 时回落到这
/// 两个常量，是一次机械改写。两个方向的成本差这么大时，先选便宜的
/// 那个。
pub const SUBCLASS_ATTRIBUTE_POINTS: u32 = 1;

/// 第一次获得某个副职时一次性授予的**技能点**数量，理由与量级权衡
/// 全部见 [`SUBCLASS_ATTRIBUTE_POINTS`]（同样是占位值、同样是引擎
/// 常量而非内容字段）。
pub const SUBCLASS_SKILL_POINTS: u32 = 1;
