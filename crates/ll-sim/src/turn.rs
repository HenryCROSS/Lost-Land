...
mod tests {
    ...
    fn 非受控实体撞上非敌对实体经turnengine结算后不移动但时钟仍前进() {
        ...
        //
        // 时钟这一半有一条**本条测不到**的边界，写在这里免得后人误以为
        // 它测到了：若占位检查退化成返回空 `Vec`，
        // `TurnEngine::perform` 的进展保证会补跑一次 `Intent::Wait`，而
        // `resolve_wait` 与占位失败分支用的是**同一个**
        // `action_cost(BASE_ACTION_COST, ..)`——两者产出的
        // `ScheduleNext` 逐位相同，坐标也都不变。**在经过 `perform` 的
        // 这条链路上没有任何断言能区分它们**，下面那条
        // `effects_seen > 0` 也不例外：它只证明这一步确实产出了效果，
        // 不证明效果是谁产的。
        //
        // 真正守住「不能靠进展保证兜底」的是 `crate::resolve` 里那条
        // 直接调 `resolve()`、根本不经过 `perform` 的单元测试
        // （`目的地站着别的实体时移动不产出moveto但仍推进时钟`）——
        // 那里没有进展保证可依赖，效果必须由占位检查自己产出。
        // Arrange
        let mut world = test_world();
        let npc = spawn_at(&mut world, (5, 5), 10);
        let neighbour = spawn_at(&mut world, (6, 5), 10);
        ...
        assert!(
            effects_seen > 0,
            "这一步必须产出效果；效果由谁产出（占位检查还是进展保证）本条区分不了，见上方注释"
        );
    }
