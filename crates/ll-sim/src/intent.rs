...
pub enum Intent {
    ...
    /// 实例的调用方（demo/未来的交互层）~~。
    ///
    /// # 【2026-08-30 重写理由】上一句划掉的部分曾指向已删除的 `examples/`
    ///
    /// 「demo」指的是 `crates/ll-sim/examples/p5_coordinate_acceptance`，
    /// 而 `examples/` 已被所有者裁定整体删除（ADR 0030）。理由段指向
    /// 一个不存在的东西，于是「这条 Intent 今天该不该留」没有判据——
    /// 2026-08-29 的文档—代码一致性审计把它连同
    /// `crates/ll-world/src/interior.rs` 整个模块判成缺陷第 5 项，
    /// 结论留给所有者。**所有者裁定：留着，重写理由。**
    ///
    /// 重写后的完整三要素（理由 / 落地条件 / 最小形状）写在
    /// `crates/ll-world/src/interior.rs` 模块文档「今天为什么留着它」
    /// 一节，**不在这里复述**——那是这条链上唯一的真相源，两处各写一份
    /// 迟早分叉。这里只留三条与本变体直接相关的事实：
    ///
    /// 1. **它不是没有消费者。** `crates/ll-sim/tests/coordinate_layers_e2e.rs`
    ///    由那个被删的 demo 逐字搬迁而来（批次 13），进出 `Interior`
    ///    的断言一条未改，今天仍在跑——demo 没了，它的验收断言还在。
    /// 2. **缺的是生产侧的产出者**：`crates/ll-game/src/` 下
    ///    `SpaceId`/`interior` 零命中，没有任何生成器造得出 `Interior`
    ///    实例，因此今天没有内容可进。这是**刻意**的等待状态，不是
    ///    漏接线。
    /// 3. **等哪一批**：P9「据点扩张 / 建筑种类与资源循环」。今天
    ///    `ll_world::settlement` 只有一种建筑（5×5 外廓、内部直接画在
    ///    地表格子上），「建筑种类」一开工就要回答「二楼 / 地窖怎么
    ///    表示」，那正是 `Interior` 的定义域。**在那之前不要接。**
    EnterSpace {
        /// 发起者。
        actor: EntityId,
        /// 目标 `Interior` 实例。
    ...
    /// `resolve.rs` 模块文档）。
    ///
    /// 与 [`Intent::EnterSpace`] 是同一条链的两端，「今天为什么留着」
    /// 的理由/落地条件/最小形状一并见该变体文档与
    /// `crates/ll-world/src/interior.rs` 模块文档。**本变体比
    /// `EnterSpace` 多一条今天就成立的前置约束**：
    /// `ll_world::state::WorldState::surface_profile` 必须在开放这条
    /// Intent 之前被显式赋值，否则退出后的 `Space::Surface.profile`
    /// 会指向一个可能未注册的占位索引（该字段文档写死了这条）。
    ExitSpace {
        /// 发起者。
        actor: EntityId,
    },
