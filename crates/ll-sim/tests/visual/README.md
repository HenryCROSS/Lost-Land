...
``baseline/p3_acceptance.png`` 是**视觉回归基准**：640×360，P3 回合与战斗验收 demo 的一帧——地形、四个单位（其中
重点目标 `boss_idle_0` 占 2×2 格，是整张遗留共享画布里**唯一**一个非 1×1
占地的条目）、伤害飘字都在画面内。
...
**`baseline/p3_acceptance.png`** 的唯一生产者是 ``ll-sim:p3_acceptance`（按 F2 存图）`。
...
```bash
git log --oneline --diff-filter=D -- crates/ll-sim/examples
git show <那个提交>^:crates/ll-sim/examples/p3_acceptance/main.rs
```
