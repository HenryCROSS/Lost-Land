...
///
/// # 第十四次重冻的原因（P6 第三批：装备槽位）
///
/// `WorldState::hash` 新增混入了每个 `Agent` 的 `equipment`
/// （`BTreeMap<EquipSlot, ItemStack>`，紧邻上一批插入的 `inventory` 之
/// 后）——见 `crates/ll-world/src/state.rs` `hash()` 对应位置新增的
/// `hasher.write_u64(agent.equipment.len() as u64)`/逐条写入槽位下标
/// 与 `write_item_stack` 的循环。本文件 `setup` 生成的
/// `player`/`enemy` 都不持有任何已装备物品，`intent_stream` 也没有任何
/// `Intent::Equip`/`Intent::Unequip`，新字段因此恒为空，但新增字段
/// 意味着喂进哈希器的字节流本身变长了（多出一个长度为零的 `u64`），
/// 摘要因此改变，与前十三次重冻同一条先例。
///
/// 人工核验（真实执行，三次独立 `cargo test` 进程）：
/// 1. 先在改动后的代码上把这条测试单独跑了两次（两次独立的
///    `cargo test` 进程），确认新摘要 `9_575_508_475_089_449_199`
///    在两次独立进程里稳定复现（不是一次性偶然值）。
/// 2. 再把 `state.rs` `hash()` 里新增的 `agent.equipment` 那一段混入
///    代码临时注释掉重新跑这条测试，摘要精确回到本次重冻之前的旧常量
///    `5_311_272_733_871_972_559`（与上面记录的第十三次重冻结果
...
///    偶然值），才把它写进下面的常量。
/// # 暴击迁进判定系统批次：**没有重冻，但基准不变的理由换了**
///
/// 这一段不记一次重冻，记的是一次「本来该变、实测没变」，以及为什么
/// 下一个人不能再把「暴击不影响这条回放」当成结构性事实。
///
/// `ll_sim::resolve::resolve_attack` 的暴击判定从
/// `chance(幸运 × 5‰, 1000)` 换成了一次 `3d20` 对抗判定（项目所有者
/// 裁定，基准暴击率 5%，见 `ll_sim::combat::CRIT_BASE_CHECK_MODIFIER`）。
/// 本回放的意图流里有一次 `Intent::Attack`，攻击者幸运是
/// `BaseStats::BASELINE.luck`（`0`）：
///
/// - **旧模型下摘要不变是一条定理**：`chance(0, 1000)` 恒返回 `false`，
///   零幸运的角色永远不可能暴击，与随机流取到什么值无关。
/// - **新模型下摘要不变是一条实测**：零幸运对应 `4.84%` 的暴击率，这
///   一下**可能**暴击，只是这个 `(种子, 实体, 时钟)` 三元组上没有。
///
/// 因此本批次 `EXPECTED_REPLAY_DIGEST` 一个字未改，且**已实跑验证**：
/// 改动后跑这条测试直接通过（不是先红再改数）。
///
/// 「没变」不等于「碰不到」——为了排除「其实根本没走到暴击那条分支」
/// 这种假绿，做了一次反向探针：把 `is_critical` 临时短路成恒真、其余
/// 改动原样保留，摘要立刻变成 `18_159_751_888_997_543_276`（测试转
/// 红）。这证明本回放确实穿过暴击判定，且暴击一旦命中就会改变摘要；
/// 探针撤掉后摘要精确回到 `13_415_522_993_411_599_503`。
///
/// 给下一个人的提醒：此后任何一次改到攻击者幸运、暴击基准偏移、
/// `CHECK_DICE`、或者这条回放的种子/时钟的改动，都可能让那 `4.84%`
/// 翻面，从而**在没有新增哈希字段的情况下**改变本常量。那不是缺陷，
/// 但必须按本文件顶部的规矩逐条说明，不能静默改数。
...
/// # 本次重冻的原因（NPC 生成批次）
///
/// `WorldState::hash` 新增混入了**顶层**字段 `materialized_settlements`
/// （已物化 NPC 的据点 id 集合，见其字段文档「参与 hash()」一节）。混入
/// 代码在 `for agent in self.actors.iter()` 循环体之外、每次 `hash()` 都
/// 会执行，因此哪怕本回放一座据点都没物化过（本文件的世界由 `setup` 手
/// 工构造，根本没有编年史），也会多写一个长度前缀 `0`——摘要必然改变。
///
/// 本回放全程没有任何据点物化（`intent_stream` 只有
/// `Move`/`Wait`/`Attack`/`OpenDoor`，`setup` 也不装编年史），变的**只是**
/// 那段长度前缀，不是行为漂移——这一点由下面第 2 步实测钉死。
///
/// 人工核验（真实执行，非由脚本自动回填）：
/// 1. 改动完成后先跑一次，确认这条测试确实红了，且报出的 `right` 正是旧
///    常量 `8_248_219_424_168_966_831`（基线确实是它，不是记忆）。实测
///    `left: 17219713135340447375`。
/// 2. 把 `state.rs` `hash()` 里新增的那三行临时换成一行不混入哈希的等价
///    读取（`let _ = &self.materialized_settlements;`），其余全部改动
///    （`WorldState` 新字段、两个查询/写入方法、存档重映射的显式
///    `materialized_settlements: _`、往返测试夹具、`ll_mod::roster`
///    整个新模块、六条新职业内容）原样保留——摘要**精确回到旧常量**
///    `8_248_219_424_168_966_831`（测试转绿，实测通过）。这一步同时证明
///    两件事：新摘要的变化只由那三行引起；本批次的其余改动没有夹带任何
///    行为漂移。
/// 3. 撤掉那三行之后，在改动后的代码上把这条测试单独跑了两次（两次
///    **独立的 `cargo test` 进程**），确认新摘要
///    `17_219_713_135_340_447_375` 在两次独立进程里稳定复现，才把它写进
///    下面的常量。
///
/// 旧值（NPC 生成批次之前）：8_248_219_424_168_966_831
///
/// # 世界生成参数落地批次：又一次重冻，同型理由
///
/// `WorldState` 新增顶层字段 `terrain_shape`（地形形态参数），
/// `WorldState::hash` 通过 `write_terrain_shape` 把它的四个字段全部
/// 混入。混入点在 `for agent in ..` 循环体之外，每次 `hash()` 必执行，
/// 摘要必然改变。
///
/// 四步核验（真实执行，非由脚本自动回填）：
/// 1. **确认基线红**：改动落地后跑
///    `cargo test -p ll-sim --test replay`，本条测试红，报出的 `right`
///    正是旧常量 `17_219_713_135_340_447_375`（基线确实是它，不是
///    记忆），实测 `left: 14731332643995045404`。
/// 2. **把改动关掉，确认精确回到旧值**：只把 `ll-world` 的
///    `WorldState::hash` 里那一行
///    `write_terrain_shape(&mut hasher, self.terrain_shape);` 注释掉，
///    其余全部改动（`GenParams` 拆出 `TerrainShape`、
///    `TileableNoise::shrink_continents`、新增字段与其序列化、四档
///    形态预设、配置接线）原样保留——摘要**精确回到旧常量**
///    `17_219_713_135_340_447_375`（测试转绿，实测通过）。这同时证明
///    两件事：新摘要的变化只由那一行引起；本批次其余改动在默认参数下
///    没有改动任何一格地形、任何一条模拟结果。
/// 3. **恢复那一行**，摘要稳定落回步骤 1 实测的新值。
/// 4. **两个独立进程复现**：在改动后的代码上把这条测试单独跑了两次
...
/// 逐位相同，因此常量本身不需要更新。
///
/// # 气候条带批次（规格 §7.1，2026-08-27）：**重冻了**
///
/// 与上面几条「没有重冻」相反，这一次两个来源都实打实动到了本条的世界：
///
/// 1. 本条的世界由 `WorldState::new(layout, &params, ..)` 真实生成地形，
///    而 `height_to_terrain` 的第四段现在按气候带三选一（见
///    `ll_world::climate`）；
/// 2. `write_terrain_shape` 多混入了 `TerrainShape::climate_band_width`。
///
/// 四步重冻（每一步真实执行）：
///
/// 1. **基线红**：`left: 763876732813032932, right: 14731332643995045404`。
/// 2. **关掉后精确回到旧值**：把 `TerrainShape::DEFAULT_CLIMATE_BAND_WIDTH`
///    临时改成 `0`（精确恒等，见其字段文档），且此时 `write_terrain_shape`
///    尚未混入新字段——本条**当场转绿**，摘要与旧常量
///    `14_731_332_643_995_045_404` 逐位相同。
///
///    这一步在本文件抓到了一处真实缺陷：新增的两种地形起初被插在
///    `lostland:snow` 之后，把其后九种地形（`floor_wood` 起）的
///    `ContentIndex` 整体平移了 2，于是即便带宽为零本条也回不到旧值
///    （实测 `2601151203819202378`）。把两条注册挪到 `materialize_base_terrain`
///    的**末尾**之后，本条精确回到旧值——「新增内容必须追加在末尾」
///    这条纪律这一次是被这条黄金基准当场咬住的，不是靠人记得。
/// 3. **恢复**：带宽改回 250，`climate_band_width` 混入
///    `write_terrain_shape`。
...
/// # 用反例证过
///
/// `write_item_stack` 新增混入了 `stack.owner`。本文件 `setup` 生成的
/// 两个实体 `inventory`/`equipment` 均为空、`ground_items` 也从未被
/// 本文件的意图流填充（意图流只有移动/攻击/等待，没有拾取、丢弃，也
/// 没有任何一方真的死掉），`write_item_stack` 的三个调用点一次都不会
/// 执行。
///
/// **实测的反例**：把 `write_item_stack` 函数体开头临时插一行
/// `hasher.write_u64(0xDEAD_BEEF)`，本文件七条**仍然全绿**、摘要逐位
/// 不变——证明本条测试的世界里根本不存在任何一个 `ItemStack`。随后
/// 删掉那一行恢复原状。
///
...
/// 四步：① 基线红，left = 11222878776777704235、
/// right = 4180595409733934027；② 把 `state.rs` 里那一行整段注掉（其余
/// 改动全部保留），本条**绿**、摘要精确回到旧值 4180595409733934027
/// ——「把改动关掉能精确回到旧值」这条纪律成立，改动来源确定；
/// ③ 恢复；④ 两个独立进程复现新值。
///
/// **本文件的世界里没有任何势力**（`setup` 只造两个实体，不跑编年史），
/// 因此摘要变化只来自那个长度 0 标记。势力表**内容**进哈希这件事由
/// `crates/ll-world/tests/determinism.rs` 的
/// `新增势力后世界哈希必须变化` / `势力覆灭与否会改变世界哈希` 两条
/// 覆盖（各带反例验证），本条不冒充覆盖它。
///
/// # 据点建筑类型批次（街道与家具，2026-08-31）：**本条没有重冻**，
/// # 用两条反例证过
///
/// 本批次动了据点的建筑布局（间距改成按人口分档 + 每三栋插一条街）
/// 与「世界生成期就摆家具」两样东西，改完本条**当场就是绿的**。
/// 按交接文档点名的第二类陷阱（「基线没红是因为那个测试的世界里根本
/// 不存在这类对象」），两条都用反例证伪，逐条实测：
///
/// - **反例 A（据点）**：往 `ll_world::settlement::stamp_settlement` 开头
///   插一行「每次铺设都往窗口左上角写一堵石墙」，本文件七条仍然全绿
///   ——本条的 `setup` 只造两个实体、从不跑编年史，一次据点铺设都不
///   发生。
/// - **反例 B（家具）**：往 `write_item_stack` 开头插
///   `hasher.write_u64(0xDEAD_BEEF)`，本文件七条仍然全绿——这个世界里
///   一个 `ItemStack` 都没有（与归属批次当初的结论一致，这次重新实测）。
///
