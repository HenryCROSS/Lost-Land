...
const DEFAULT_MENU_BINDINGS: &[KeyBinding] = &[
    // ── 两个「任何时候都能按」的键（规格 N13 / D8 / D9） ──────────
    //
    // 它们此前**只绑 `Gameplay`**，而一开模态屏上下文就切到 `Menu`，
    // 于是两个键在模态屏里都解析不出任何动作：
    //
    // - Tab 能开菜单、关不掉菜单——开关键不对称（D8）；
    // - F2 在任何模态屏上都截不了图（D9），而
    //   [`crate::input::GameKey::Screenshot`] 的文档明说这个键是「冻结
    //   视觉回归基准的入口，不是调试功能」——结果所有 UI 屏都进不了
    //   视觉回归基准。
    //
    // 补在这张表的**最前面**，与下面那批导航键分开：它们不是「菜单
    // 导航」的一部分，是「哪个上下文都该有」的一对。
    KeyBinding {
        key: KeyCode::Tab,
        modifiers: Modifiers::NONE,
...
const DEFAULT_TEXT_ENTRY_BINDINGS: &[KeyBinding] = &[
    // 截图键在这里也要有（规格 N13 / D9）——见
    // [`DEFAULT_MENU_BINDINGS`] 开头那段。
    //
    // **Tab 刻意不补**：规格 N13 原文「Tab 在文本输入态里不补——它将来
    // 是『跳到下一个输入框』」。这不是漏了，`keybind.rs` 的测试里有一条
    // 断言盯着它今天确实没绑。
    KeyBinding {
        key: KeyCode::F2,
        modifiers: Modifiers::NONE,
...
#[cfg(test)]
// 这个模块的测试住在隔壁 `keybind_tests.rs`，用 `#[path]` 挂进来。
// 本文件在行数棘轮的快照里（`scripts/ci/file_size_budget.json`），而它
// 承担的是**四件事**：键位词汇表（`KeyCode`/`Modifiers`）、三张默认
// 绑定表、`KeyBindings` 这个类型本身、以及默认值合并算法。八百行测试
// 与那四件事都不是同一件事，搬出去之后加一条新按键不再要跟行数预算
// 打架——本批（规格 N13）补两条绑定就撞上了这堵墙。
// 手法与 `ll_game` 的 `app_tests.rs`、`ll_ui` 的 `render_layout_tests.rs`
// 一样：纯搬家，一个字没改，`use super::*;` 因此照旧看得见私有项。
